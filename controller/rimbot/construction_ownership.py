...
def owned_buildings(plan, facts):
    ...
    for step in plan.spec.steps:
        ...
        if (step.source != 'AUTOPILOT' or goal is None or goal.source != 'AUTOPILOT' or goal.cancelled
                or progress.state != 'complete' or not isinstance(step.action, (Buildings, RoomShell))):
            continue
        ...
        for i, placement in enumerate(placements):
            ...
            if (not row or row.get('stage') != 'built' or row['present'] is not True or row.get('blocker') is not None
