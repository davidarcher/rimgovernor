    ...
    async def ask(self, role, context, contract, thinking=True):
        ...
        if contract is ObjectiveProposal:
            ...
            instructions=('You are a colony department manager. Propose semantic objectives from your assigned task and observed state. '
                          ...
                          'Every growing objective must specify one crop_def and target_cells as the TOTAL desired colony-wide zone area for that crop, counting existing zones. Choose this bounded target from food reserves, demand, climate and labor. Do not equate unsown fields with absent fields. Existing queued orders are not complete. Address concrete blockers and preserve useful ongoing work. '
        ...
        instructions += ('\nconstruction_work is a native observation of queued sites, remaining materials, current pawn job targets and unforced worker eligibility. '
                         ...
                         'Read project.progress.sites for the current recovery state. needs_alternative means a definition restriction; priorities cannot fix it. Cancel obsolete project blueprints using native cancellation and inspect before placing an eligible replacement. targeted means a pawn has a job target, not that work has advanced. Fix observed blockers before adding redundant orders. If next_offset is present, further sites exist; absence from the first page does not mean missing. '
