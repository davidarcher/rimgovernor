...
async def test_no_site_does_not_call_executor(monkeypatch):
    ...
    rt=NS(memory={'spatial_layout':{'regions':[{'id':'farm','project_ids':['food'],'purpose':'farm'}],'deferred':{'room':'Invalid room footprint'}},'projects':[project]},resume_initial_planning=AsyncMock(),check_generation=Mock(),mode='automate',note=Mock(),persist=Mock(),manager_context=AsyncMock())
