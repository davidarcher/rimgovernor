...
async def test_fresh_quicktest_with_identical_map_metadata_resets_all_state(colony):
    ...
    assert {k:v for k,v in rt.memory.items() if k not in ('world_facts','risk_state','food_samples')} == rt.empty_memory()
...
async def test_compaction_retains_observed_building_materials(colony):
    ...
    async def complete(messages,*args):
        ...
        assert retained['observed_building_definitions']==[]
        assert json.loads(messages[-1]['content'])['items']==[definition]
