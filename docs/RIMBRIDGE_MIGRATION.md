...
### Automatic rendering demand (2026-09-08)

Normal rendered launches now install a render-demand driver on the first controller poll. A visible non-minimized Windows game window keeps rendering. Otherwise a bounded real-time lease controls camera rendering and MapDrawer.DrawMapMesh; cameras disabled by the driver are restored on wake. Geometry updates, initialization, game UI, simulation, time controls and frame caps are not patched. Unknown window/platform state fails open. Startup headless mode remains separate and cannot render.

The dashboard sends independent viewer heartbeats only when its Colony view is visible and video is playing. Pause video, hidden tabs and navigation release demand; closed/lost tabs expire. Backend viewer expiry is eight seconds, and a native eight-second lease expires even if the controller dies (up to roughly sixteen seconds after the last browser heartbeat). With no viewers the controller stops screenshot capture even if the game window is visible. Explicit visual-review capture renews a fifteen-second lease and waits briefly for a frame. Other clients calling raw screenshot tools are outside this controller's lease path.

Validation: native build/install, 137 controller tests, six dashboard tests, TypeScript and dashboard build pass. Live Windows test observed minimized/no-viewer suspended=true; viewer lease woke rendering and produced cameraVersion=1; expiry suspended again; restoring the window resumed rendering. No simulation-speed benchmark or full-headless parity claimed. This is a lighter rendering suspension path, not runtime graphics teardown. Installed with game closed; rendered controller restored in Manual mode.

### Render suspension acceptance and timing (2026-09-08)
