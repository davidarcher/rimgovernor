...
| Knowledge and persistence | `knowledge.py` retrieves local strategy cards; `wiki.py` retrieves reference material. `memory.py` stores advisory colony notes. `review_evidence.py` retains exact review-local results in memory. `store.py` persists state, events and retired action/method evidence in SQLite; its bounded compressed decision-snapshot helpers currently have no runtime callers. |
