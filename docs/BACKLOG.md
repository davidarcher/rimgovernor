...
- [ ] **G01.05 — Routine planning integration across the workflows below.**
  ...
  **Implementation first; native acceptance at the end.** Defer new in-game tests
  ...
  - [ ] **05.6 — Management and service recovery (G01.07e).**
    ...
    behind `--routine-animal-containment-plans`. `RecoverDisasterServices`'s
    repair/breakdown/refuel service-job dispatch is now code-complete too:
    domain/policy/store/executor/buildingruntime vertical plus
    `RoutineRecoveryPlanner` behind `--routine-recovery-plans`, dispatching
    only `RecoveryServiceProposal` candidates from
...
- [ ] **G01.07f — World progression works through Go.**
  ...
  is implemented and tested, player-command submission/planning is wired
  (`store.SubmitCaravanDeparture`/`Player.SubmitCaravanDeparture`, one-action plan
  like `SubmitBuilding`/`SubmitDraft`), and `buildingruntime.CaravanDepartureBoundary`
  now implements `InspectCaravanDeparture`/`DepartCaravan`/`ObserveCaravanDeparture`
  and is wired via `session.go`'s `EnableCaravanDeparture`, so the executor can
  reach native dispatch. `RemainingHomeColonists`/`HomeFoodRunwayDays` come from
