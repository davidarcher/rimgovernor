...
| --- | --- | --- |
| `AllowStartingSupplies`: discovered Unforbid, eight original cells, `supply_batches.supply_rectangles`, then wait | `policy/starting_supplies.go`, routine/store cohort history; native supply snapshots and guarded Allow/readback boundary | Go typed Allow action, journal, handler and service admission remain gated. Map retained cells to exact native item snapshots; observe original cells allowed and do not renew later player forbidding. First startup consumer: b. Native gameplay acceptance remains deferred to G01.12. |
