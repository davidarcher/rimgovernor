...
All roles default to Qwen 3.5 4B, including strategy, daily planning, arbitration and one-project task planning. The local trial uses a 65,536-token load with four concurrent predictions; the controller still compacts conversation history at 60,000 characters. Departments propose objectives, the administrator reconciles them, and task planners inspect and draft commands for one native system. Uncertain objectives should be deferred with a concrete missing fact, not sent to a larger model. There is no larger-model escalation path. The administrator accepts or defers objectives. Task planning respects the reasoning setting. The historical Executor role label refers to task planning; issuing its approved batch remains ordinary controller code without an additional model call. The department parallelism setting defaults to four and can be set to 1–4. Only objective reviews run concurrently. Their snapshot is shared; later execution refreshes live state. Model progress retains role/model labels, and the dashboard lists active departments.
...
Before strategic review and execution, projects receive current native observations. Growing progress lists zone IDs, crop, area, plants present, sowing enabled and growth. A model-selected crop_def and target_cells specify total colony-wide designated capacity for one crop; every creation call re-reads existing zones and rejects expansion beyond the remaining capacity. Unplanted cells still count as placed zones. Changing the strategy can change the target; existing zones are never deleted automatically.

Construction progress reports built/frame/blueprint counts inside the reserved room and roof coverage separately. It does not infer sleeping capacity or room quality merely from enclosure completion. Previous free-text feedback is archived in events and removed from current context when refreshed. Explicit stockpile creation/configuration wording routed to another system is rejected for administrator correction; this is a narrow mismatch check, not a general natural-language intent classifier. Supply access and storage execute before growing and construction.

Validation: 175 regression tests passed before the final architect-routing filter; a live read-only probe saw 249 rice cells plus two potato cells and blocked extra rice against a 249-cell target. The room probe reported built walls/door and 56/56 roofed interior cells. These checks do not establish successful autonomous starter-base completion.
