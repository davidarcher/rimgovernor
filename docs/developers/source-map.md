...
| Dashboard | [dashboard/src](../../dashboard/src) is the React/TypeScript UI; it detects the Go backend (`GET /api/health`) and renders structured observation/player controls. [dashboard/timeline.html](../../dashboard/timeline.html) is the offline case timeline over an acceptance output directory ([measure throughput](testing/measure-throughput.md#case-timeline)); [dashboard/src/features/governor](../../dashboard/src/features/governor) is the live Governor view over `/api/telemetry` ([dashboard](architecture/dashboard.md#telemetry)). |
