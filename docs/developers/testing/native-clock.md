...
Run `go run ./internal/nativeaccept/cmd/tickbudgetaccept -root <prepared-root> -output
<new-directory>` from `go/`. Each speed/budget case reloads the unchanged baseline. The
native companion must independently pause at 1, 37 and 600 ticks at Normal, Fast and
Superfast, retain its deadline across renewal, and remain paused afterward. The probe
also checks external native speed/pause commands, lease expiry before the deadline and
retirement on load changes. Add `-rendered` for a visible game.
