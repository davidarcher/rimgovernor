...
| Raider cover inside the engagement zone (#581): on the committed layout the fixture stages three mature oaks and two granite chunks on the approach ahead of Entry within the defenders' range; the defense census names each as cover with a CAS token, the layout planner (every tier standing) recomputes the approaches and orders one `cover_clearance` method (`defense-cover-<tick>`) with a cut on each tree and a haul on each chunk through the game's own designators, the plan completes with every thing gone from its cell and natively none of the staged things stands, no colonist drafted | `go run ./internal/nativeaccept/cmd/acceptance run defense/cover -root <abs root> -output <fresh dir> -rimgovernor <path to rimgovernor.exe>` from `go/` against a `DefenseFixture,GuardedConstructionFixture` build; `result.json` carries `cover_staged`, `cover_method`, `cover_plan` (`targets`: thing to designation) and `inspect_after_cover` | Serve-driven on the committed layout checkpoint with the `defensive-layout` family; no raid is staged, so the arrivals are empty and the sectors rank by path length alone. About 5-10 minutes: the trees must actually be felled and the chunks hauled. Rerun when `routine_defense_cover.go`, `policy/defense_arrivals.go`, the coverclearance boundary, `NativeClearCover.cs`, `NativeDefenseObservationTools.cs` or `RaidArrivalState.cs` changes. |
