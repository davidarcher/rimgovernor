...
| Shared generated request models | `go run ./cmd/contractgen -root .. -check contracts/generation.json` from `go/`; `python scripts/check_placement_requests.py --check` from root. | Go CI also compiles and tests C# with pinned .NET/NuGet inputs. See [generation](../../../contracts/schema-generation.md). |
