...
`defense/raid`, `defense/raid-bypass`, `defense/raid-breach`, `defense/siege`, `defense/drop`, `defense/predator`, `defense/hive`, `defense/shippart`, `defense/cover` and `defense/turrets` cases declare
...
| Raider cover inside the engagement zone (#581, #620): on the committed layout the fixture stages three mature oaks, two granite chunks, a lone granite rock and an unowned wall segment on the approach ahead of Entry within the defenders' range, then a small ground raid walking in toward Entry; the service fights the raid from the line (ActiveCombat recovers), the native raid trail records a ground arrival whose crossing ranks a sector, the defense census names each staged thing as cover with a CAS token, the layout planner (every tier standing) recomputes the approaches and orders `cover_clearance` methods (`defense-cover-<tick>`) with a cut on each tree, a haul on each chunk, a mine on the rock and a deconstruct on the wall through the game's own designators, the plans complete with every thing gone from its cell, the planner's approach summary shows `arrivals` and `ranked_sector_raids` above zero, and natively none of the staged things stands, no colonist drafted | `go run ./internal/nativeaccept/cmd/acceptance run defense/cover -root <abs root> -output <fresh dir> -rimgovernor <path to rimgovernor.exe>` from `go/` against a `DefenseFixture,GuardedConstructionFixture` build; `result.json` carries `cover_staged`, `raid_incident`, `cover_raid`, `cover_method`, `cover_plans`, `cover_arrivals` and `inspect_after_cover` | Serve-driven on the committed layout checkpoint with the `defensive-layout` family. About 5-10 minutes: the raid must be fought and the things actually felled, hauled, mined and deconstructed. Rerun when `routine_defense_cover.go`, `policy/defense_arrivals.go`, the coverclearance boundary, `NativeClearCover.cs`, `NativeDefenseObservationTools.cs` or `RaidArrivalState.cs` changes. |
...
| A heat wave over an enclosed sleeping room with Cooler research and a fuelled generator to spare (#406): the temperature family admits one `Cooler` through a wall cell of the room (cold side in, hot side outdoors), native cooling takes the room under the hot exit and the goal recovers; the game then runs to three days after the fixture with no Heatstroke hediff on any colonist, and the independent read finds the one cooler connected, powered and at a setpoint under the hot exit while the outdoors still reads above it | `acceptance run temperature/heatwave -root <abs root> -output <fresh dir> -rimgovernor <abs exe>` from `go/` against a `ForecastFixture,RoutineSleepingFixture` build; `result.json` carries `prepared` (waves, snaps, the colonists' safe maximum and the day's targets), `cooler_cell`, `cooler_completed_tick`, `temperature_recovered_tick`, `heatstroke_*` audits and `coolers_after`/`facts_after` | Serve-driven (temperature, power, work). The fixture dresses the colonists in dusters and hats and stacks fully ramped `HeatWave`/`ColdSnap` conditions so the day's mean outdoors sits near 40 C (peak 47, trough 33: above the hot exit, under the dressed colonists' safe maximum); it refuses a roll whose margins do not fit. Its conduits are `HiddenConduit` so the power family's Zzztt upgrades (#405) do not rework the net under the cooler. Rerun when `temperature_method.go`, `routine_temperature.go`, the cooler wall preview or `ForecastFixture.HeatWave` change. |
| Colony extent runtime and explicit expansion (#519, #580): complete geometry establishes history; the ready corridor is restored by Home maintenance; adding and removing an expansion area preserves every Home bit | `acceptance run upkeep/colony-extent -root <abs root> -output <fresh dir> -rimgovernor <abs exe>` against `SleepingFixture,HomeCoverageFixture,UpkeepFixture,ForecastFixture,RoutineSleepingFixture`; `result.json` records extent cell counts and `home_mask_byte_identical` | Ready connected rooms with one missing corridor cell, no construction wait. The case exercises the authenticated expansion API across controller restarts and reads the entire native Home mask before, after growth and after removal. |
| Connected autonomous Home (#452): two routine-built beds in roofed chambers joined by a walled corridor; actual corridor cells become Home while an outdoor cell does not; stale geometry/revision refuse writes; a removed corridor cell is restored across controller restarts and native Home survives save/load | `acceptance run upkeep/home-coverage -root <abs root> -output <fresh dir> -rimgovernor <abs exe>` against `UpkeepFixture,ForecastFixture,RoutineSleepingFixture`; results include `beds_built`, `stage_home_0`, `stale_guards`, `removed`, `stage_home_1`, `save_recovery_home` | Ready connected-room fixture; controller builds both beds through ordinary pawn work. Bounded Home writes then run over the same journal. Pure native geometry probes cover disconnected/outdoor cells and three batches over 700 cells; Go tests cover ownership, recurrence and selection after blocked targets. Stockpile receipt ownership remains `upkeep/storage-missing`. |
| Raid-point turret scaling (#341): native threat and routine projection agree before and after tripling a selected loose-stock bundle; more turret positions are proposed on the same firing geometry with an unchanged armed roster | `acceptance run defense/layout-stocked` against a `DefenseFixture` build on the committed defense-layout save | Paused, five-minute budget, no construction or combat waits. The fixture normalizes steel/components/silver to 2000/100/20000, calibrates native difficulty once to 290 raid points, then triples those counts without changing difficulty. Native power, costs, stock, site cells and lines of fire gate both proposals. Nightly executes the case. |
