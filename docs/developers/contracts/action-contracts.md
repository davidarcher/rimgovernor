...
| `claim_building` (PatchBuilding claim) | CAS-gated claim of one exact claimable building for the player (#459): `Building.ClaimableBy(player)` then `SetFaction(player)`, the game's own Claim gizmo rule; the token covers the faction and, for a casket, whether it holds anything. Read back as `player_owned` on the building listing row, which lists the target without the player-only filter. Nothing opens a casket. |
| `open_casket` (PAWN_ORDER_KIND_OPEN_CASKET) | Ordered vanilla `Open` job by one eligible colonist, drafted or not, on one exact filled `Building_AncientCryptosleepCasket` under its claim token (#460); native adds the `Open` designation under authority right before the job and removes it if the job drops with the casket still full. Completion is the casket observed empty after the job; opening one casket ejects every casket of its shrine group, so a plan carries one order. Refused when the casket is empty, reserved, unreachable at its interaction cell or the pawn cannot do dumb labor. Behind `--routine-shrine-open-caskets`. The optional heat variant (`PAWN_ORDER_KIND_OPEN_CASKET_HEAT`) uses one ordinary `AttackStatic` bullet volley from a door cell outside the interior. It requires an owned draft, measured room heat above 60 C, no colonists inside, line of sight and a damage margin above the casket explosion threshold. It never designates Open or teleports the opener; completion still requires the exact casket observed empty. |
...
| `Arrest` | Vanilla custody through `JobDefOf.Arrest`: an armed, violence-capable arrester with an owned draft claim, an exact target still in a mental state or a standing neutral `Faction.OfAncients` humanlike, and an exact usable prisoner bed. Native eligibility, manipulation, reservation and reach are rechecked; hostile aggressive states such as Berserk are refused. Optional pawn/target/bed tokens fence stale snapshots. Completion requires a later observation of the living target as a colony prisoner in that exact bed with its mental state ended, under the original order and draft claim. Arrest resistance or interrupted delivery is unsuccessful; admission, mental-state recovery alone and custody before bed delivery do not complete it. `mood/arrest` proves refusals, replay and a legal sad-wander arrest. |
...
| Claim (`PatchBuilding` claim) | `Exact claimable building is unavailable.` (NotFound); `Building is already the player's.` / `The game refuses the claim (contents, faction or a spawn lock).` (InvalidRequest); `Building snapshot changed; observe before new admission.` (StaleIdentity). |
...
| Bed assignment (`AssignBed`) | `Current map required.`; `Pawn unavailable for bed assignment.` / `Bed unavailable.` (NotFound; the pawn's current job, whoever ordered it, is no refusal, #461); `Native bed assignment eligibility refused.`; then `Pawn snapshot changed` / `Bed snapshot changed; observe before new admission.` (compared only when sent; execute omits the tokens, #244). |
| Home extension (`ExtendHome`) | `bounded visible native facility geometry is unavailable` (NotFound; the exact building/stockpile is gone, forbidden, burning or fogged); `the facility footprint changed since it was read` (shape hash of the next batch, at most 256 cells from the building and connected enclosed roofed rooms); `the Home area changed since it was read` (revision advanced by Set, Clear and Invert). Preview reports a failed rule as `accepted: false`. Execute restores the batch's missing Home cells synchronously. Progress checks the admitted batch still belongs to the connected geometry and is actually Home, independently of the next batch. `changed_cells`, `covered` and the post-write revision are native evidence. |
