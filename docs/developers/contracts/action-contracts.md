...
| Hunt (`AcquireResource` on an animal) | `two hunts are already outstanding on this map`; `a roof collapse is pending on this map`; `the exact animal is no longer spawned on this map` (NotFound); `the animal is dead or downed`; `the animal is neither safe wild prey nor a recognised pest`; `the animal is not at the expected cell`; `the animal's corpse is not the expected resource`; `the animal's cell is fogged`; `the animal is already designated for hunting`; `the native hunt designator refuses the animal`; `no usable butcher bill with an assigned cook accepts the corpse`; `no free colonist with hunting enabled and an ordinary ranged weapon has a safe route to the animal`; `the animal snapshot changed since it was read`. |
...
| Home extension (`ExtendHome`) | `bounded visible native facility geometry is unavailable` (NotFound; the building or `stockpile:<id>` is gone, forbidden, burning, fogged or over 256 cells); `the facility footprint changed since it was read` (the shape hash of the footprint, the building plus adjacent enclosed roofed rooms of at most 128 cells); `the Home area changed since it was read` (the map-wide Home revision, advanced by every player Set, Clear and Invert). No per-target token exists; the two hashes are the whole check. Preview reports a failed rule as `accepted: false` with the reason, and the missing cells are added in the same call (a synchronous area write), so the receipt's `home` evidence carries `changed_cells`, `covered` and the revision after the write. |
