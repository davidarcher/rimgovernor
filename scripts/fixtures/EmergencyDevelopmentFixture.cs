...
namespace HomeBridge.BridgeTools
{
    ...
    public sealed class EmergencyDevelopmentFixture
    {
        ...
        public async Task<object> Setup(IRimBridgeContext ctx, CancellationToken cancellationToken,
            [ToolParameter(Description = "stocks, opponents, wound, low-health, unavailable-doctor, external-order")] string op,
            [ToolParameter(Description = "Exact colonist identity for pawn cases.", DefaultValue = "")] string pawn = "")
        {
            return await ctx.MainThread.InvokeAsync<object>(() => {
                ...
                if (op == "stocks") {
                    foreach (var item in new[] { ("WoodLog", 2500), ("Steel", 1200), ("ComponentIndustrial", 40), ("MealSimple", 200), ("MedicineHerbal", 30) }) {
                ...
                return new { success=true, op, pawn, tick=Find.TickManager.TicksGame,
