    ...
    async with bridge_session(gabs_executable(root),configuration) as bridge:
        ...
        try:
            for speed in ('Normal','Superfast','Ultrafast'):
                for repeat in range(repeats):
                    ...
                    row={'completed':completed,'speed':speed,'repeat':repeat+1,'ticks':after['ticksGame']-before,'elapsed_seconds':elapsed,
                        'tps':(after['ticksGame']-before)/elapsed,'rendering':'headless' if headless else 'normal','dashboard_captures':False,'native_receipt':receipt}
            ...
            try:
                ...
                (root/('speed-benchmark-headless.json' if headless else 'speed-benchmark.json')).write_text(json.dumps(rows,indent=2),encoding='utf8')
...
if __name__=='__main__':
    ...
    asyncio.run(main(args.seconds,args.repeats,args.headless))
