...
async def run(args):
    ...
    config = prepare(root)
    report = dict(passed=False, model_calls=0, checks=[], samples=[])
    ...
    async with bridge_session(gabs_executable(root), config) as bridge:
        ...
        async def refresh():
            ...
            facts['longTermMedical'] = care_state(people)
        ...
        try:
            ...
            setup = (await bridge.call('test/medical_management_setup')).structuredContent
