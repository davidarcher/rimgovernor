...
async def run(args):
    ...
    report['parameters'] = dict(production=args.production, production_only=args.production_only, seconds=args.seconds)
    ...
    try:
        ...
        if args.production or args.production_only:
            ...
            report['production_setup'] = preparation
            # Bills uses Thing.ThingID; gear observations use GetUniqueLoadID.
            report['recipes_before_research'] = await rt.game.invoke('home/bills', dict(action='recipes',
                bench=preparation['bench'].removeprefix('Thing_'), dryRun=True))
            report['production_research'] = await fixture('production_research')
            ...
            report['production_initial'] = facts
            ...
            try:
                ...
                report['production_selection'] = goal.evidence
            ...
            while time.monotonic()-started < args.seconds * 4:
                ...
                if fresh_worn:
                    ...
                    assert all(not w['forced'] for w in fresh_worn)
                    async with rt.lock:
                        rt.batch = await observe(rt.game)
                        rt.reconcile_plan()
                    if report['production'].get('wear_step'):
                        progress = rt.current_plan.progress[report['production']['wear_step']]
                        assert progress.state == 'complete', progress
                        report['production']['completed'] = progress.model_dump()
                    break
