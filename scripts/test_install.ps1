...
$dll = 'C:/Program Files (x86)/Steam/steamapps/common/RimWorld/Mods/RimBotObservations/BridgeTools/Observations/RimBot.Observations.BridgeTools.dll'
$backup = Join-Path $repo ('.rimbot/install-backup-' + [guid]::NewGuid().ToString() + '.dll')
...
try {
    ...
    Copy-Item -LiteralPath integrations/colony-bridge/BridgeTools/Observations/RimBot.Observations.BridgeTools.dll -Destination $dll -Force
