...
| ChoiceDialogTool.cs (`NativeChoiceDialogOperations`) | AnswerDialog | Colony-wide like ConfirmColonyNames: the exact observed `window_id`, `option_index` and `option_label` of the open `Verse.Dialog_NodeTree` (research finished, caravan demands, quest choices the game force-pauses on) stand in for the snapshot token, refusing on any drift, a disabled option, or a hyperlink option that does not resolve the dialog. Activation runs the option's own native `action` (the same delegate a click runs); `DialogEffect` reports whether the window closed or advanced to a further node, and never fabricates the consequence of the choice. The Go side answers under the `AnswerDialog` routine goal with `--routine-dialog-prefer` (label substrings in preference order, default `OK,Ignore,Keep playing,Give`; first selectable otherwise); the `dialog/pause` acceptance case covers it (#156). |
