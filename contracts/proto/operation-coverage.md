...
| NativePlantAcquisition.cs | CancelAcquisition | Exact source/resource/cell of a plant `AcquireResource` this controller still tracks; removes the harvest/cut designation under a fresh attempt of the same action. Applied evidence is the acquisition effect with `designated=false`; the record then observes unsuccessful (designation gone before harvest). A pending plant effect carries `pending_reason` (forbidden, below harvest growth, no enabled cutter, unreachable, cutters busy) (#291). |
...
| NativeArrestOperations.cs | Arrest | Exact arrester/target/prisoner-bed identities, optional snapshot tokens, owned draft and authority admission. Native armed/violence/manipulation/arrest/bed/reach checks; normal targets and hostile Berserk are refused. Uses vanilla Arrest including resistance and prisoner conversion. Progress requires the living target's mental state ended in the exact prisoner bed; receipt replay never repeats the job. Covered by `mood/arrest`. |
...
| OrderTool.cs | SetDrafted/MovePawn/AttackTarget/PawnTargetOrder | Equip/rescue/capture/tend/haul/work/repair/clean/open_casket, native job readback (`NativeOpenCasketOperations.cs`: the vanilla Open job on one filled ancient casket under its claim token, the Open designation added under authority and removed if the job drops, the opener may be drafted); exact owned draft cleanup, preserve player orders. Resolve is observation. |
...
| ChoiceDialogTool.cs (`NativeChoiceDialogOperations`) | AnswerDialog | Colony-wide like ConfirmColonyNames: the exact observed `window_id`, `option_index` and `option_label` of the open `Verse.Dialog_NodeTree` (research finished, caravan demands, quest choices the game force-pauses on) stand in for the snapshot token, refusing on any drift, a disabled option, a hyperlink option, an option that neither resolves the tree nor links to a further node (its action opens another window, such as CaravanMeeting's Trade opening `Dialog_Trade`, which stays with the player), or a dialog still inside its `delayInteractivity` grace. Activation runs the option's own native `action` (the same delegate a click runs); `DialogEffect` reports whether the window closed or advanced to a further node, and never fabricates the consequence of the choice. Each option also carries `keys`, the Keyed translation keys of the active or English language whose text produces its label, so policy is language-neutral. The Go side answers under the `AnswerDialog` routine goal with `--routine-dialog-prefer` (patterns in preference order, each an exact translation key or a label substring, default `OK,Ignore,CaravanMeeting_MoveOn,GameOverKeepWatching,CaravanDemand_Give`; the first selectable resolving option otherwise), waits out the interactivity delay, and follows a linked node as a further answer; the `dialog/pause` acceptance case covers it (#156, #179). |
| ColonyNamingTool.cs (`NativeColonyNamingOperations`) | ConfirmColonyNames | Colony-wide, not entity-scoped: exact observed window_id/faction_name/settlement_name stand in for EntityPrecondition/snapshot token, refusing on any drift. Reuses the legacy tool's own dialog lookup, field reflection and native name validators/callbacks. Autopilot-eligible via the ordinary authority `WritePrecondition`; independent of `PlayerPresentation.Apply`'s own confirm_colony_names branch, which stays explicit-player-only (see presentation-coverage.md). The Go side confirms under the `ConfirmColonyNames` routine goal (`naming` family); the `naming/confirm` acceptance case covers it (#178). |
...
`home/dialog_text`, player_input, render_demand/video_stream/pawn_image, upstream camera/selection/letter/UI interactions are packaging's explicit player/presentation contracts. `home/confirm_colony_names` is split: its autopilot-eligible path is `Operations.ConfirmColonyNames` above; `PlayerPresentation.Apply`'s own confirm_colony_names branch is a distinct, explicit-player-only path that packaging owns. `play_until_event`, save/load/start/stop/time speed are root's clock/lifecycle. All55 owned production exports are partitioned by the source inventory; no fixture exports, god mode, debug acceleration, instant research/health/inventory mutation or arbitrary SDK/gizmo/tool execution enters these operation variants.
