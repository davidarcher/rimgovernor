...
| PopulationTool.cs | SetPrisonerInteraction | AttemptRecruit/MaintainOnly ordinary interaction; no instant recruitment. |
...
`home/confirm_colony_names`, `home/dialog_text`, player_input, render_demand/video_stream/pawn_image, upstream camera/selection/letter/UI interactions are packaging's explicit player/presentation contracts. `supervised_play`, `play_until_event`, save/load/start/stop/time speed are root's clock/lifecycle. All55 owned production exports are partitioned by the source inventory; no fixture exports, god mode, debug acceleration, instant research/health/inventory mutation or arbitrary SDK/gizmo/tool execution enters these operation variants.
