...
and `HarmonyAssembly` pointing to private licensed inputs. Run its net472
executable with:

```powershell
dotnet build contracts/tests/NativeContractProbes.csproj -p:HarmonyAssembly=<0Harmony.dll> -p:RimWorldManagedDir=<Managed dir>
dotnet run --project contracts/tests/NativeContractProbes.csproj --no-build -p:HarmonyAssembly=<0Harmony.dll> -p:RimWorldManagedDir=<Managed dir> -- native-explosive-causality ^
  <RimGovernor.Bridge.dll path> <Runtime directory> <SDK directory> <game managed directory> <Harmony directory>
```
