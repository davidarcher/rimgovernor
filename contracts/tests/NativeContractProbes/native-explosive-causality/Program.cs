...
internal static class NativeExplosiveCausalityProgram
{
    ...
    static void Set(object value,string n,object? data)
    {
        ...
        var method=new DynamicMethod("SetFixtureField",typeof(void),new[]{typeof(object),typeof(object)},typeof(NativeExplosiveCausalityProgram),true);
    ...
    static IEnumerable<CodeInstruction> RecordFactoryArgs(IEnumerable<CodeInstruction> instructions,MethodBase __originalMethod)
    {
        ...
        code.Add(new CodeInstruction(OpCodes.Call,AccessTools.Method(typeof(NativeExplosiveCausalityProgram),nameof(CaptureFactoryArgs))));code.AddRange(instructions);return code;
    }
    ...
    static void Run()
    {
        ...
        sink.Patch(Method("NativeLaunch"),prefix:new HarmonyMethod(typeof(NativeExplosiveCausalityProgram),nameof(LaunchSink)){priority=Priority.Last});
        sink.Patch(Method("DamageTarget"),prefix:new HarmonyMethod(typeof(NativeExplosiveCausalityProgram),nameof(DamageSink)){priority=Priority.Last});
        sink.Patch(factory,transpiler:new HarmonyMethod(typeof(NativeExplosiveCausalityProgram),nameof(RecordFactoryArgs)){priority=Priority.Last});
        ...
        sink.Patch(spawn,prefix:new HarmonyMethod(typeof(NativeExplosiveCausalityProgram),nameof(SpawnSink)){priority=Priority.Last});
        sink.Patch(AccessTools.DeclaredMethod(typeof(GenExplosion),"CalculateNeededLOSToCells"),prefix:new HarmonyMethod(typeof(NativeExplosiveCausalityProgram),nameof(LosSink)){priority=Priority.Last});
        ThingDefOf.Explosion=Raw<ThingDef>();ThingDefOf.Explosion.defName="Explosion";
        sink.Patch(start,prefix:new HarmonyMethod(typeof(NativeExplosiveCausalityProgram),nameof(StartSink)){priority=Priority.Last});
        sink.Patch(Method("ExplosionCellTarget"),prefix:new HarmonyMethod(typeof(NativeExplosiveCausalityProgram),nameof(CellSink)){priority=Priority.Last});
        ...
        foreach(var kind in new[]{"instigator","definition","args"})
        {
            ...
            var mutation=AccessTools.Method(typeof(NativeExplosiveCausalityProgram),kind=="args"?nameof(SubstituteArgs):nameof(SubstituteDamage));sink.Patch(Method("DamageTarget"),prefix:new HarmonyMethod(mutation){priority=Priority.First});
...
internal static class NativeExplosiveCausalityProbe
{
    internal static int Invoke(string[] args)
    {
        try
        {
            ...
            typeof(NativeExplosiveCausalityProbe).Assembly.GetType("NativeExplosiveCausalityProgram",true)!.GetMethod("Start",BindingFlags.Static|BindingFlags.NonPublic)!.Invoke(null,new object[]{args[0]});return 0;
        }
