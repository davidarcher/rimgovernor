...
longer has its own `.csproj`. Build the merged project and run it with:

```powershell
dotnet run --project contracts/tests/NativeContractProbes.csproj -- native-combat-operations <args...>
```

with the private compiled bridge DLL and dependency directories (game managed,
RimBridgeServer SDK, Harmony, and private runtime if separate) forwarded as
`<args...>`. The executable loads actual compiled adapters and official generated
Protobuf messages.
...
authority hooks and operation envelope neighbors (now other probe names within the
same `NativeContractProbes.csproj`). Native gameplay acceptance is required
separately; these contract and guard checks do not establish combat.
