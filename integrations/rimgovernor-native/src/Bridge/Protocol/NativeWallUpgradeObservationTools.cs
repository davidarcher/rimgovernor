...
namespace HomeBridge.BridgeTools
{
    ...
    //    one row per admissible normal, target == original;
    ...
    public sealed class NativeWallUpgradeObservationTools
    {
        ...
        private static bool Interior(Map map, IntVec3 inside)
        {
        ...
        private static Building? ColonistWall(Map map, IntVec3 cell)
        {
        ...
        private static Obs.WallUpgradeSite? Replacement(Map map, Building wall, IntVec3 normal, List<Obs.WallMaterialOption> materials, Common.ObservationContext context)
        {
            var origin = wall.Position;
            var outside = origin + normal;
            if (!Interior(map, origin - normal) || !outside.InBounds(map) || outside.GetRoom(map)?.TouchesMapEdge != true) return null;
            ...
            if (cells.Any(c => !c.InBounds(map) || c.Fogged(map) || !c.Standable(map) || map.zoneManager.ZoneAt(c) != null
                ...
                || c.GetThingList(map).Any(t => t is Building || t is Blueprint || t is Frame || t is Plant || t.def.category == ThingCategory.Item))) return null;
            string? blocker = null;
            ...
            var row = Site(map, wall, wall, normal, cells, context);
            if (blocker != null) row.Blocker = blocker;
            row.ReplacementMaterials.AddRange(materials);
            return row;
        }

        private static Obs.WallUpgradeSite? Cleanup(Map map, Building permanent, IntVec3 normal, Common.ObservationContext context)
        {
            ...
            var row = Site(map, backups[0], permanent, normal, cells, context);
            row.Replacement = NativeBuildingObservationTools.Project(permanent);
            foreach (var backup in backups) row.CompletedBackups.Add(NativeBuildingObservationTools.Project(backup));
            var blocker = RoofSupportSafety.Blocker(backups[0], out _);
            if (blocker != null) row.Blocker = blocker;
            return row;
        }

        private static Obs.WallUpgradeSite Site(Map map, Building target, Building original, IntVec3 normal, List<IntVec3> cells, Common.ObservationContext context)
        {
        ...
        private static List<Obs.WallMaterialOption> Materials()
        {
