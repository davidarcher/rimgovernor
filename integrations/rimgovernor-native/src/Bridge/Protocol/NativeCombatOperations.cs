...
namespace HomeBridge.BridgeTools
{
    ...
    internal sealed class NativeCombatRecord
    {
        ...
        {this.identity=identity;this.pawn=pawn;this.target=target;this.job=job;jobId=job.loadID;jobDef=job.def.defName;
            ...
            this.precondition=precondition.Clone();this.authority=authority;damage=NativeCombatCausality.Track(Current.Game,pawn,target,job,DamageGuard);}
        ...
        private bool DamageGuard()
        {
            try {
                if(!NativeCombatCausality.IsReady || job.loadID!=jobId || job.def?.defName!=jobDef || !ReferenceEquals(job.targetA.Thing,target)
                    ...
                    || snapshot.Facts.DraftRevision!=before.Facts.DraftRevision || before.Facts.OrderRevision==ulong.MaxValue)return false;
                return snapshot.Facts.OrderRevision==before.Facts.OrderRevision+1
                    || dispatching && snapshot.Facts.OrderRevision==before.Facts.OrderRevision;
            }catch{return false;}
        }
        ...
        internal Receipts.Progress Observe(Common.AttemptKey attempt,Common.ObservationContext context)
        {
            ...
            try {
                ...
                if(phase==NativeCombatPhase.Unknown){result.Unknown=new Receipts.UnknownEffect {Reason="No causally attributed combat outcome or exact live attack job is available."};return result;}
    ...
    internal static class NativeCombatOperations
    {
        ...
        private static bool Legal(Operations.AttackTarget command,Pawn pawn,Pawn target,out JobDef? definition)
        {
            definition=null;
            ...
            bool ranged=command.Mode==Operations.AttackMode.Ranged || command.Mode==Operations.AttackMode.Auto && FloatMenuUtility.UseRangedAttack(pawn);
            if(ranged) {
                if(!FloatMenuUtility.UseRangedAttack(pawn) || pawn.equipment?.PrimaryEq?.PrimaryVerb==null
                    || !pawn.equipment.PrimaryEq.PrimaryVerb.CanHitTarget(target))return false;
        ...
        private static bool Resolve(Operations.AttackTarget command,Common.ObservationContext context,out NativeControlIdentity identity,
            out Pawn? pawn,out Pawn? target,out NativePawnSnapshot? snapshot,out JobDef? definition,out Common.Failure failure)
        {
            identity=new NativeControlIdentity(Current.Game,Find.CurrentMap,context.Identity.ColonyId,context.Identity.LoadToken);
            pawn=null;target=null;snapshot=null;definition=null;
            ...
            if(!NativePawnControlState.IsReady || !NativeCombatCausality.IsReady)return false;
            ...
            if(command.Mode==Operations.AttackMode.Ranged || command.Mode==Operations.AttackMode.Auto && FloatMenuUtility.UseRangedAttack(pawn))
            {failure=ProtoBoundary.Fail(Common.FailureCode.Unsupported,"Ranged attack requires a projectile lineage producer; this adapter supports Melee and Auto resolving to melee.");return false;}
            ...
            if(check!=NativePawnControlResult.Ready){failure=NativeDraftProtocol.Failure(check,context);return false;}
            if(!Legal(command,pawn,target,out definition)){failure=ProtoBoundary.Fail(Common.FailureCode.InvalidRequest,"Native attack weapon, reach, target or requested combat predicates refuse this order.");return false;}
            return true;
        }
        internal static Operations.ExecuteReply Execute(NativeOperationState state,Operations.ExecuteRequest request,Common.ObservationContext context)
        {
            ...
            try {
                if(!Resolve(command,context,out var identity,out var pawn,out var target,out var before,out var definition,out var failure))return new Operations.ExecuteReply {Failure=failure};
                ...
                if(!Resolve(command,context,out identity,out pawn,out target,out before,out var rechecked,out failure))return new Operations.ExecuteReply {Failure=failure};
                if(rechecked!=definition || !NativeMovementOperations.Owns(before!,owner))return Refuse(Common.FailureCode.OwnerConflict,"Attack prerequisites changed before admission.");
                ...
                using(authority.Owned()) {
                    if(!Resolve(command,context,out var finalIdentity,out var finalPawn,out var finalTarget,out var checkedPawn,out rechecked,out failure)
                        || !ReferenceEquals(finalPawn,pawn) || !ReferenceEquals(finalTarget,target) || rechecked!=definition || !NativeMovementOperations.Owns(checkedPawn!,owner))
        ...
        internal static Operations.PreviewReply Preview(Operations.AttackTarget command,Common.ObservationContext context)
        {
            ...
            try {
                if(!Resolve(command,context,out _,out _,out _,out var snapshot,out var definition,out var failure))return new Operations.PreviewReply {Failure=failure};
