...
namespace RimBot.Colony
{
    public static class ActivitySummary
    {
        public static string Tool(ToolCall call, string result, bool success)
        {
            ...
            try
            {
                switch (call.Name)
                {
                    ...
                    case "manager_save_plan": return "Updated the shared plan.";
                    default: return Short(result, 180);
                }
            }
        ...
        public static string Model(string text)
        {
            ...
            if (trimmed.StartsWith("{") || trimmed.StartsWith("[") || trimmed.StartsWith("```"))
            {
                try {
                    ...
                    if (!string.IsNullOrWhiteSpace(summary)) return Short(summary, 180);
                } catch { }
                return "";
            }
            return Short(trimmed, 180);
        }
        ...
        public static string Short(string text, int max)
        {
            ...
            int end=text.LastIndexOf(' ',max-1,max);
            return text.Substring(0,end>max/2?end:max-1)+"…";
        }
    }
}
