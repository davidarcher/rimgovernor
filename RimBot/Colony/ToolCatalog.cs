...
namespace RimBot.Colony
{
    public static class ToolCatalog
    {
        ...
        public static List<ToolDefinition> Definitions() => new List<ToolDefinition>
        {
            ...
            Define("manager_save_plan", "Save a short shared plan for the next review. Include unfinished orders and what to wait for. Maximum 1200 characters. Refer to construction by site coordinates; native IDs change at each construction stage. A response containing only this tool ends the review.", "{plan:{type:'string',maxLength:1200}}", "plan"),

            // Architect and construction
            Define("architect_preview", "Preview proposed buildings/lines on the map without placing orders. Native placement checks and a compact overlay let you revise the whole layout before spending materials. No shelter templates or automatic gap filling.", "{placements:{type:'array',minItems:1,maxItems:32,items:{type:'object',properties:{defName:{type:'string'},material:{type:'string'},x:{type:'integer'},z:{type:'integer'},rotation:{type:'integer',minimum:0,maximum:3},toX:{type:'integer'},toZ:{type:'integer'}},required:['defName','x','z','rotation'],additionalProperties:false}}}","placements"),
            Define("architect_build", "Architect > Build: use native placement validation and designation. Buildings with zero construction work are placed immediately, as in the game UI. Other buildings become construction orders. Supply exact discovered definition/material names. Optional toX/toZ drag a horizontal/vertical line, inclusive. Preview multi-cell layouts with architect_preview first.", "{defName:{type:'string',description:'Exact defName returned by architect_buildables, e.g. Bed'},material:{type:'string',description:'Required for material-based buildings: exact material returned by architect_buildables, as returned by architect_materials. Omit for buildings with requiresMaterial=false.'},x:{type:'integer'},z:{type:'integer'},rotation:{type:'integer',minimum:0,maximum:3},toX:{type:'integer'},toZ:{type:'integer'}}", "defName","x","z","rotation"),
            ...
            // Pawns and work
            Define("equipment_equip", "Pawn right-click > Equip WEAPON only. For clothing/armor, use pawns_orders(pawnId,targetId) and the returned Wear actionId with pawns_order. Uses native checks and ordered job. Works drafted or undrafted; permits the native Allow behavior. Native confirmation dialogs require player interaction.", "{pawnId:{type:'integer'},itemId:{type:'integer'}}", "pawnId","itemId"),
            ...
            Define("pawns_orders", "Supply pawnId and targetId OR both x,z for the full native right-click menu: prioritized construction/work, hauling, equipment, clothing, ingestion, rescue, combat and available mod/DLC options, including disabled reasons. Pass a returned actionId to pawns_order.", "{pawnId:{type:'integer'},targetId:{type:'integer'},x:{type:'integer'},z:{type:'integer'}}","pawnId"),
            ...
            Define("map_inspect", "Inspect at most 8x8 cells. x,z is the lower left corner; map coordinates are absolute. Read before placing orders. Use format=grid for a compact overhead view up to 32x32 with native room and roof facts; default detailed cells remain limited to 8x8.", "{x:{type:'integer'},z:{type:'integer'},width:{type:'integer',minimum:1,maximum:32},height:{type:'integer',minimum:1,maximum:32},format:{type:'string',enum:['cells','grid']}}", "x","z","width","height"),
