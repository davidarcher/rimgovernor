...
func TestRoutineFamilyFilesCoverFamilies(t *testing.T) {
	...
	shared := map[string]bool{"routine_census.go": true, "routine_yield.go": true, "routine_work_projects.go": true, "routine_sleeping.go": true}
...
func TestSelectScopesRoutineFamilies(t *testing.T) {
	...
	if !slices.Equal(sel.Cases, []string{"condition", "lifecycle", "light"}) {
