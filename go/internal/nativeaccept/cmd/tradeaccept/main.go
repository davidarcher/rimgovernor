// Command tradeaccept exercises the OpenTrade vertical (G01.07f) end to end
...
func main() {
	...
	output := flag.String("output", "", "fresh output directory (default <root>/native-trade-accept-acceptance)")
	...
	if *output == "" {
		*output = *root + "/native-trade-accept-acceptance"
	}
	...
	report := na.NewReport("Native OpenTrade vertical only (per go/README.md's documented TradeSetLines/"+
		...
		"stale-identity refusals, and replay idempotency.", !*rendered)
