// Command settlementgiftaccept exercises the GiftCaravanSilver vertical
...
func main() {
	...
	output := flag.String("output", "", "fresh output directory (default <root>/native-settlement-gift-acceptance)")
	...
	if *output == "" {
		*output = *root + "/native-settlement-gift-acceptance"
	}
	...
	report := na.NewReport("Native GiftCaravanSilver vertical: a real gift-mode TradeSession/TradeDeal "+
		"execution against an actual visited settlement, exact caravan-position/faction-token CAS and "+
		"visit-first refusal, replay idempotency and a post-gift stale-faction-token refusal.", !*rendered)
...
func run(ctx context.Context, root, output, gameID string, headless bool, report na.Report) error {
	...
	prepared, err := h.Call(ctx, "prepare", "test/settlement_gift_prepare", map[string]any{"silverCount": 100})
	if err != nil {
		return err
	}
	if success, _ := na.AsBool(prepared["success"]); !success {
		return fmt.Errorf("settlement_gift_prepare refused: %#v", prepared)
	}
	if na.AsString(prepared["colonyId"]) != na.AsString(identity["colonyId"]) ||
		...
		return fmt.Errorf("settlement_gift_prepare identity does not match the fresh debug game")
	}
