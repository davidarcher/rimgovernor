...
func run(ctx context.Context, root, output, gameID string, headless bool, report na.Report) error {
	...
	prepared, err := h.Call(ctx, "prepare", "test/building_temperature_prepare", map[string]any{})
	if err != nil {
		return err
	}
	if success, _ := na.AsBool(prepared["success"]); !success {
		return fmt.Errorf("building_temperature_prepare refused: %#v", prepared)
	}
	if na.AsString(prepared["colonyId"]) != na.AsString(identity["colonyId"]) ||
		...
		return fmt.Errorf("building_temperature_prepare identity does not match the fresh debug game")
	}
