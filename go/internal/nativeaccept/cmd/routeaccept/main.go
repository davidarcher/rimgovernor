...
func run(ctx context.Context, root, output, gameID string, headless bool, binary string, report na.Report) error {
	...
	if !na.Contains(names, "test/routes_prepare") {
		return fmt.Errorf("missing test/routes_prepare in discovery; rebuild the native mod with -Fixture RoutesFixture")
	}

	prepared, err := h.Call(ctx, "prepare", "test/routes_prepare", map[string]any{})
	if err != nil {
		return err
	}
	if success, _ := na.AsBool(prepared["success"]); !success || !na.MatchesIdentity(prepared, identity) {
		return fmt.Errorf("routes_prepare refused or identity mismatch: %#v", prepared)
	}
	...
	service, err = na.LaunchService(ctx, cfg, gabsExecutable, na.ServiceLaunch{Binary: binary, Families: []string{"routes", "work"}, Extra: []string{"--clock-speed", na.ClockSpeed()}}, report)
	if err != nil {
		return err
	}
	defer service.Stop()
