...
func TestParseSuiteResolvesRegistry(t *testing.T) {
	...
	if want := []string{self, "run", "smoke/identity", "-root", worker, "-output", opts.Output, "-game", "rimgovernor-trial", "-fresh", "-checkpoint-every", "0", "-series", opts.Series, "-budget", "4m0s"}; strings.Join(argv, " ") != strings.Join(want, " ") {
	...
	if want := []string{self, "run", "light/dark", "-root", worker, "-output", opts.Output, "-game", "rimgovernor-trial", "-fresh", "-checkpoint-every", "0", "-series", opts.Series, "-rimgovernor", "rg.exe", "-budget", "4m0s"}; strings.Join(argv, " ") != strings.Join(want, " ") {
