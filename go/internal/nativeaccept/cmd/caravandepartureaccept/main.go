// Command caravandepartureaccept exercises the CaravanDeparture vertical
...
const sessionOwner = "native-caravan-departure-acceptance"

func main() {
	root := flag.String("root", "", "absolute disposable worker root (e.g. .rimgovernor/bridge)")
	output := flag.String("output", "", "fresh output directory (default <root>/native-caravan-departure-acceptance)")
	...
	if *output == "" {
		*output = *root + "/native-caravan-departure-acceptance"
	}
	...
	report := na.NewReport("Native CaravanDeparture vertical: an actual FormCaravan dispatch through the "+
		"real Dialog_FormCaravan mechanism, CaravanCatalog cargo/route resolution, home-staffing admission "+
		"refusal, replay idempotency and a post-departure stale-catalog refusal.", !*rendered)
...
func run(ctx context.Context, root, output, gameID string, headless bool, report na.Report) error {
	...
	prepared, err := h.Call(ctx, "prepare", "test/caravan_departure_prepare", map[string]any{"crewCount": 1})
	if err != nil {
		return err
	}
	if success, _ := na.AsBool(prepared["success"]); !success {
		return fmt.Errorf("caravan_departure_prepare refused: %#v", prepared)
	}
	if na.AsString(prepared["colonyId"]) != na.AsString(identity["colonyId"]) ||
		...
		return fmt.Errorf("caravan_departure_prepare identity does not match the fresh debug game")
	}
