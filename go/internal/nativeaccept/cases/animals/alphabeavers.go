...
// alphabeaverFamilies serve the pest hunt (acquisition), the hunters'
// staging (equip arms the best shooters from the baseline's loose bows,
// work gives ranged pawns Hunting; #321) and the emergency responders: a
// wounded beaver has a 50% chance of turning manhunter, and the whole pack
// is a herd, so a hunt can turn into a small raid the defense family has to
// answer before the hunt census offers the survivors again.
const alphabeaverFamilies = "acquisition,equip,work,supply,defense,tend,rescue"

// alphabeaverWindow is how long the colony gets to arm and assign its
// hunters and ClearPests to plan and dispatch a hunt of every beaver: two
// hunts outstanding at a time, each following its animal across the map.
// Three beavers took two in-game days on a loaded host (runs 6 and 7 of
// #247), so the pack is two.
...
func init() {
	cases.Register(cases.Case{
		...
		Reason: "arms and assigns hunters from the baseline's loose bows, then hunts across a 250x250 map, at most two outstanding at a time, each following its beaver, then a bounded native settle for a beaver already designated",
		Run: func(ctx context.Context, s cases.Session) error {
			...
			_, err := sustainedfood.Observe(ctx, s, sustainedfood.Observation{
				...
				Prepare: func(ctx context.Context, h *na.Harness, report na.Report) error {
					...
					// Nobody is staged as a hunter: the equip and work
					// families have the loose bows to make one.
					if len(na.AsSlice(prepared["hunters"])) == 0 && len(na.AsSlice(prepared["looseWeapons"])) == 0 {
						return fmt.Errorf("no colonist holds a ranged weapon and none lies loose; nothing can arm a hunter: %#v", prepared["colonists"])
					}
					return nil
				},
...
// beaver is dead or gone from the map, no wild alphabeaver remains, a
// colonist was armed and assigned as a hunter by the families (not the
// fixture), and no colonist died to the pack. A beaver still alive but
// designated when the watch ended is given a bounded settle: the goal did
// its part, the hunter finishes it under native ticks alone.
func auditAlphabeavers(ctx context.Context, s cases.Session, h *na.Harness, report na.Report, staged []string) error {
	...
	for _, raw := range na.AsSlice(inspect["colonists"]) {
		...
		if hunter, _ := na.AsBool(row["hunter"]); hunter {
			hunters++
		}
	}
	if hunters == 0 {
		return fmt.Errorf("no colonist holds an ordinary ranged weapon with Hunting active; the equip and work families never staged a hunter: %#v", inspect["colonists"])
	}
	return nil
}
