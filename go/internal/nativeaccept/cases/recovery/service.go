// The recovery/service case exercises the disaster-recovery service
...
const serviceOwner = "native-recovery-service-acceptance"

func init() {
	cases.Register(cases.Case{
		Name: "recovery/service",
		Scope: "Native RecoverService (disaster-recovery service) dispatch: a genuinely damaged " +
	...
	buildRequest := func(actionID string, generation any, operation map[string]any) map[string]any {
		return map[string]any{
			"precondition": map[string]any{
				...
				"attempt": map[string]any{"controllerSessionId": serviceOwner, "actionId": actionID, "attemptId": "1"},
			},
			"operation": operation,
		}
	}
