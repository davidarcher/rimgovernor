// The lifecycle/checkpoint case is the native-acceptance run for G01.09's
// trusted native save-checkpoint slice: three rimgovernor/lifecycle_save
// outcomes against the real native tool -- a paused happy-path completed save with identity/tick/
...
func runCheckpoint(ctx context.Context, s cases.Session) error {
	report := s.Report()
