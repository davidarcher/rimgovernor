// The construction/guarded-completed and guarded-cancelled cases prove disposable ordinary WoodLog Wall construction under guarded
...
const guardedOwner = "native-guarded-construction-acceptance"
...
// guardedPlaceRequest builds an operations_execute placeBuilding request at grant's
...
func guardedPlaceRequest(identity, grant map[string]any, number int, site map[string]any) map[string]any {
	...
	return map[string]any{
		"precondition": map[string]any{
			...
			"attempt": map[string]any{
				"controllerSessionId": guardedOwner,
...
// BuildingSmokeEnv names a prebuilt cmd/buildingsmoke executable; unset,
// the case builds one under its output directory.
const BuildingSmokeEnv = "RIMGOVERNOR_BUILDINGSMOKE"

func init() {
	for _, outcome := range []string{"completed", "cancelled"} {
		cases.Register(cases.Case{
			Name: "construction/guarded-" + outcome,
			Scope: "Disposable ordinary WoodLog Wall construction, guarded authority and attempt " +
...
// buildingSmokeBinary is the cmd/buildingsmoke executable the Go phases
// run: BuildingSmokeEnv when set, else one built into output.
func buildingSmokeBinary(ctx context.Context, output string) (string, error) {
	if binary := os.Getenv(BuildingSmokeEnv); binary != "" {
		return binary, nil
	}
	binary := filepath.Join(output, "buildingsmoke")
	if runtime.GOOS == "windows" {
		binary += ".exe"
	}
	build := exec.CommandContext(ctx, "go", "build", "-o", binary, "github.com/davidarcher/RimGovernor/go/internal/buildingruntime/cmd/buildingsmoke")
	if out, err := build.CombinedOutput(); err != nil {
		return "", fmt.Errorf("build buildingsmoke (run from inside the go module, or set %s): %w\n%s", BuildingSmokeEnv, err, out)
	}
	return binary, nil
}

// guarded is the case body for one expected Go observe outcome.
func guarded(expectedOutcome string) func(ctx context.Context, s cases.Session) error {
	return func(ctx context.Context, s cases.Session) error {
		return runGuarded(ctx, s, expectedOutcome)
	}
}

func runGuarded(ctx context.Context, s cases.Session, expectedOutcome string) error {
	report := s.Report()
	...
	supervisor := &na.ScenarioClock{Wire: wire, Identity: identity, Owner: guardedOwner, Report: report}
	...
	request := guardedPlaceRequest(identity, renewed, 1, site1)
	...
	conflict := guardedPlaceRequest(identity, renewed, 1, site2)
	...
	blockedRequest := guardedPlaceRequest(identity, renewed, 2, site2)
	...
	if err := refuse(ctx, h, "draft-blocks-new", guardedPlaceRequest(identity, supervisor.Grant, 3, site2), "FAILURE_CODE_STALE_GENERATION"); err != nil {
		return err
	}
