// Package upkeep holds issue #2's (B04h) colony-upkeep acceptance: one
// staged deficit per case, the live rimgovernor service composed with
...
type scenario struct {
	...
	// deadline is the scenario's Budget when measured: a healthy run plus
	// margin under the paused-map admission cadence (#129); cases.MaxBudget
	// otherwise.
...
func init() {
	...
	for _, name := range scenarioOrder {
		...
		cases.Register(cases.Case{
			Name: "upkeep/" + name,
			Scope: "Colony upkeep (issue #2 B04h), " + name + " deficit staged by " + sc.fixture + " on the " + sustained.BaselineSave +
				" save: the live service composed with only the owning routine families follows the durable journal from deficit through method to observed recovery, audited by an independent native read after the service releases the game.",
			Start:  cases.Save{Name: sustained.BaselineSave},
			Keep:   sc.keep,
			Serve:  &cases.ServeSpec{Families: sc.families, Extra: sc.extra, Prefix: prefix},
			Budget: sc.budget(),
...
func run(ctx context.Context, s cases.Session, sc *scenario) (err error) {
	report, h, identity := s.Report(), s.Harness(), s.Identity()
	if !na.Contains(s.Names(), sc.fixture) {
	...
	// when the case fails after the service took over the slot.
	defer func() {
		if err == nil {
			return
		}
		...
		if reattachErr != nil {
			report["upkeep_postmortem_error"] = reattachErr.Error()
			return
		}
		if upkeep, readErr := readUpkeep(stopCtx, ph, identity, "upkeep-postmortem"); readErr == nil {
			report["upkeep_postmortem"] = upkeep
		} else {
			report["upkeep_postmortem_error"] = readErr.Error()
		}
	}()
	...
	if sc.name != "fire" {
		for _, row := range review.Development.Rows {
			if row.Reason == policy.DevelopmentEmergency {
				return fmt.Errorf("first review holds development as an emergency (patients %v, hostiles unknown); the fixture roll is unusable", review.MedicalCare.Patients)
			}
		}
	}
	...
	return sc.verify(ctx, h, identity, prepared, report)
}
