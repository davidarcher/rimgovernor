...
func RunningGames(gameCopy string) ([]int, error) {
	script := "Get-CimInstance Win32_Process -Filter \"Name='RimWorldWin64.exe'\" | ForEach-Object { \"$($_.ProcessId)`t$($_.ExecutablePath)`t$($_.CommandLine)\" }"
	out, err := exec.Command("powershell", "-NoProfile", "-NonInteractive", "-Command", script).Output()
	if err != nil {
		return nil, fmt.Errorf("list RimWorld processes: %w", err)
	}
	prefix := strings.ToLower(filepath.Clean(gameCopy))
	var pids []int
...
func StopGames(ctx context.Context, gameCopy string) ([]int, error) {
	...
	if out, err := exec.CommandContext(ctx, "powershell", "-NoProfile", "-NonInteractive", "-Command", script).CombinedOutput(); err != nil {
		return nil, fmt.Errorf("stop pids %v: %s %w", pids, strings.TrimSpace(string(out)), err)
	}
