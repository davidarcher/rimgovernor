...
func TestStalledHuntActionsGracePeriod(t *testing.T) {
	...
	if got := stalledHuntActions([]domain.Progress{hunt}, sources, nil, 100+5999, 6000); got != nil {
		t.Fatal("stalled before grace elapses", got)
	}
	got := stalledHuntActions([]domain.Progress{hunt}, sources, nil, 100+6000, 6000)
	if len(got) != 1 || got[0] != "hunt-deer" {
		t.Fatal("did not report stalled hunt", got)
	}
	// An exempt thing (a downed pest, #321) is never stalled.
	if got := stalledHuntActions([]domain.Progress{hunt}, sources, map[string]bool{"deer": true}, 100+6000, 6000); got != nil {
		t.Fatal("exempt hunt stalled", got)
	}
	if got := stalledHuntActions([]domain.Progress{hunt}, sources, nil, 100+6000, 0); got != nil {
...
func TestStalledHuntActionsIgnoresResolvedAndNonHuntWork(t *testing.T) {
	...
	hunt := dispatchedHunt(t, "hunt-deer", "deer", 100)
	sources := map[string]bool{"deer": true}
	if got := stalledHuntActions([]domain.Progress{hunt}, map[string]bool{}, nil, 100+6000, 6000); got != nil {
	...
	if got := stalledHuntActions([]domain.Progress{resolved}, sources, nil, 100+6000, 6000); got != nil {
