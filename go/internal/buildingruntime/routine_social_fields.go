	...
	for _, name := range []string{"Plant_Hops", "Plant_Smokeleaf"} {
		...
		for _, d := range projection.Definitions {
			if d.Name == name {
				...
				sites := policy.PlanSocialCrop(crop, projection.CropClimate, cells, policy.FarmSiteRequest{Bounds: projection.Bounds, Anchor: projection.Center, Cells: projection.Cells, Protected: protected})
				if sites.Cells == 0 {
					continue
				}
