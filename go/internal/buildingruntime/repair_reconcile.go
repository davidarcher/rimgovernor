...
func (b *RepairBoundary) ObserveRepair(ctx context.Context, dispatch executor.RepairDispatch, current domain.GenerationSnapshot) (executor.RepairEvidence, error) {
	p := dispatch.Attempt
	out := executor.RepairEvidence{StartedAt: b.clock.Now(), Pawn: dispatch.Admission.Pawn, Structure: dispatch.Admission.Structure, Observation: domain.Observation{Action: p.Action.ID(), Attempt: p.Attempt, Snapshot: current, Effect: domain.EffectUnknown}}
	...
	if job != nil {
		if err = repairJob(job, dispatch); err != nil {
			return out, err
		}
