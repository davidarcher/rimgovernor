...
func tendDoctorFacts(pawn domain.PawnID, row *n.PawnState, token string) policy.TendDoctorFacts {
	facts := policy.TendDoctorFacts{Pawn: pawn, SnapshotToken: token, Dead: draftBool(row.Dead), Downed: draftBool(row.Downed), Drafted: draftBool(row.Drafted), MentalState: draftPresence(row.MentalState, row.Issues, "mental_state")}
