...
func (b *OpenCasketBoundary) ObserveOpenCasket(ctx context.Context, dispatch executor.OpenCasketDispatch, current domain.GenerationSnapshot) (executor.OpenCasketEvidence, error) {
	p := dispatch.Attempt
	out := executor.OpenCasketEvidence{StartedAt: b.clock.Now(), Pawn: dispatch.Admission.Pawn, Casket: dispatch.Admission.Structure, Observation: domain.Observation{Action: p.Action.ID(), Attempt: p.Attempt, Snapshot: current, Effect: domain.EffectUnknown}}
	...
	reply, _, err := b.native.ObservePawnOrderProgress(ctx, attempt, receipt)
	if err != nil {
		return out, err
	}
	...
	case *r.Progress_Pending:
		...
		if !out.Complete || job == nil || !job.GetVerified() || actual.Native != p.Snapshot.Native {
			return out, executor.ErrEvidence
		}
	...
	if job != nil {
		if err = openCasketJob(job, dispatch); err != nil {
			return out, err
		}
