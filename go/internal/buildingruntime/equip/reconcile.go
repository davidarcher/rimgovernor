...
func (b *EquipBoundary) ObserveEquip(ctx context.Context, dispatch executor.EquipDispatch, current domain.GenerationSnapshot) (executor.EquipEvidence, error) {
	p := dispatch.Attempt
	out := executor.EquipEvidence{StartedAt: b.clock.Now(), Pawn: dispatch.Admission.Pawn, Thing: dispatch.Admission.Thing, Observation: domain.Observation{Action: p.Action.ID(), Attempt: p.Attempt, Snapshot: current, Effect: domain.EffectUnknown}}
	...
	if job != nil {
		if err = equipJob(job, dispatch); err != nil {
			return out, err
		}
