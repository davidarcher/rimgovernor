...
// MineAcquisitionNative reuses bridge's generic AcquireResource wire dispatch
// (PreviewAcquisition/LookupAcquisition/ObserveAcquisition are already
// parameterized over bridge.AcquisitionTarget/AcquisitionAttempt, not tied to
// any particular ActionKind) but reads via ReadMineAcquisition
// (ListResourceSources) instead of ReadAcquisition (the AcquisitionFacts
// census).
type MineAcquisitionNative interface {
	ReadMineAcquisition(context.Context, *c.Identity, string, domain.Cell) (bridge.AcquisitionRead, bridge.Result, error)
...
type MineAcquisitionWriter interface {
	Acquire(context.Context, *a.WritePrecondition, bridge.AcquisitionTarget) (*op.ExecuteReply, bridge.Result, error)
}
type MineAcquisitionCapabilities struct {
	Native MineAcquisitionNative
	Writer MineAcquisitionWriter
}
type MineAcquisitionBoundary struct {
	*boundary.Boundary
	capabilities MineAcquisitionCapabilities
}

func NewMineAcquisitionBoundary(base *boundary.Boundary, capabilities MineAcquisitionCapabilities) *MineAcquisitionBoundary {
	return &MineAcquisitionBoundary{Boundary: base, capabilities: capabilities}
}

func (b *MineAcquisitionBoundary) InspectAcquisition(ctx context.Context, target executor.Target) (executor.AcquisitionInspection, error) {
	out := executor.AcquisitionInspection{StartedAt: b.Clock.Now()}
	acquisition, ok := target.Action.MineAcquisition()
	if !ok {
		return out, executor.ErrEvidence
	}
	read, _, err := b.capabilities.Native.ReadMineAcquisition(ctx, boundary.Identity(target.Snapshot), acquisition.Definition(), acquisition.Cell())
	if err != nil {
		return out, err
	}
	...
	preview, _, err := b.capabilities.Native.PreviewAcquisition(ctx, boundary.Identity(current), selected)
	if err != nil {
		return out, err
	}
...
func (b *MineAcquisitionBoundary) Acquire(ctx context.Context, request executor.AcquisitionDispatch) (executor.Receipt, error) {
	p := request.Attempt
	acquisition, ok := p.Action.MineAcquisition()
	return b.DispatchWrite(ctx, p,
		...
		func(pre *a.WritePrecondition) (*op.ExecuteReply, bridge.Result, error) {
			return b.capabilities.Writer.Acquire(ctx, pre, bridge.AcquisitionTarget{Acquisition: acquisition, Token: request.SnapshotToken})
		},
	)
}
func (b *MineAcquisitionBoundary) ObserveAcquisition(ctx context.Context, p executor.Placement, current domain.GenerationSnapshot) (executor.AcquisitionEvidence, error) {
	...
	lookup, _, err := b.capabilities.Native.LookupAcquisition(ctx, w)
	if err != nil {
		return out, err
	}
	...
	reply, _, err := b.capabilities.Native.ObserveAcquisition(ctx, w, admitted)
	if err != nil {
		return out, err
	}
	v := reply.GetProgress()
...
var _ executor.AcquisitionBoundary = (*MineAcquisitionBoundary)(nil)
