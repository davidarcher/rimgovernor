...
func NewCleanBoundary(native CleanNative, writer CleanWriter, leases LeaseSource, clock executor.Clock, session string) (*CleanBoundary, error) {
...
func cleanPawnFacts(pawn domain.PawnID, row *n.PawnState, token string) policy.CleanPawnFacts {
	facts := policy.CleanPawnFacts{Pawn: pawn, SnapshotToken: token, Dead: draftBool(row.Dead), Downed: draftBool(row.Downed), Drafted: draftBool(row.Drafted), MentalState: draftPresence(row.MentalState, row.Issues, "mental_state")}
...
func (b *CleanBoundary) InspectClean(ctx context.Context, target executor.Target) (executor.CleanInspection, error) {
	...
	reply, _, err := b.native.ReadPawns(ctx, boundaryIdentity(target.Snapshot), []string{string(clean.Pawn())})
	if err != nil {
		return out, err
	}
	...
	filth, _, err := b.native.ReadFilthTarget(ctx, boundaryIdentity(current), clean.Filth(), clean.Cell())
	if err != nil {
		return out, err
	}
	...
	if _, err = boundaryContext(filth.Context, current); err != nil {
		return out, err
	}
	...
	preview, _, err := b.native.PreviewPawnOrder(ctx, boundaryIdentity(current), cleanCommand(string(clean.Pawn()), clean.Filth(), pawnToken, filthToken))
	if err != nil {
		return out, err
	}
	...
	facts := policy.CleanFacts{Snapshot: current, PawnTick: domain.Tick(observed.Context.GetTick()), PreviewTick: domain.Tick(evaluated.Context.GetTick()), NativeCanTry: draftBool(job.CanTry)}
...
func (b *CleanBoundary) attempt(dispatch executor.CleanDispatch) (bridge.PawnOrderAttempt, error) {
	...
	if !ok || p.Attempt == 0 || p.Tick < 0 || admission.Snapshot != p.Snapshot || admission.Pawn != clean.Pawn() || admission.Filth != clean.Filth() || admission.Cell != clean.Cell() || admission.Tick > p.Tick || !boundaryID(admission.PawnSnapshotToken) || !boundaryID(admission.FilthSnapshotToken) {
