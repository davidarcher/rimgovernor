...
func clockCoreFixture(t *testing.T) (*ClockCoordinator, *store.Store, *clockCoreFake, store.ClockIntent) {
	...
	q, err := NewClockCoordinator(db, fake, fake, clockCoreLease{func(domain.GenerationSnapshot) (string, error) { return "lease", nil }}, ClockCoordinatorConfig{CallTimeout: time.Second, JournalTimeout: time.Second})
	if err != nil {
		t.Fatal(err)
	}
...
func TestClockCoordinatorLostStartRecoveryDisabled(t *testing.T) {
	...
	restarted, err := NewClockCoordinator(db, f, f, q.leases, q.config)
	if err != nil {
		t.Fatal(err)
	}
