...
func (b *DraftBoundary) ObserveDraft(ctx context.Context, p executor.Placement, current domain.GenerationSnapshot) (executor.DraftEvidence, error) {
	...
	out.Drafted = draftBool(row.Drafted)
	...
	original := draftReceiptJob(receipt)
...
func (b *DraftBoundary) InspectDraftCleanup(ctx context.Context, p executor.Placement, cleanup domain.DraftCleanup) (executor.DraftCleanupInspection, error) {
	...
	if claim.Action != p.Action.ID() || claim.Attempt != p.Attempt || claim.Pawn != domain.PawnID(attempt.PawnID) || claim.Origin != p.Snapshot || string(claim.Session) != b.session || !boundaryID(string(claim.Claim)) {
		return out, executor.ErrEvidence
	}
	...
	case *n.DraftClaimObservation_Owned:
		owned := state.Owned
		if owned == nil || owned.Owner == nil || owned.Owner.ControllerSessionId == nil || owned.Owner.PlayerDirection == nil || owned.Owner.GetPlayerDirection() == 0 || !boundaryID(owned.Owner.GetControllerSessionId()) || !boundaryID(owned.GetClaimId()) || !proto.Equal(owned.PawnSnapshot, row.Pawn.Snapshot) {
			return out, executor.ErrHeld
		}
...
func (b *DraftBoundary) ReleaseDraft(ctx context.Context, release domain.DraftRelease) (executor.DraftCleanupReceipt, error) {
	...
	q := release.Request
	claim := q.Claim
	if release.Sequence == 0 || q.Tick < 0 || q.Observed.Validate() != nil || q.Observed.Native == 0 || claim.Origin.Validate() != nil || claim.Origin.Native == 0 || claim.Origin.Direction == 0 || claim.Attempt == 0 || !boundaryWorld(q.Observed, claim.Origin) || q.Observed.Native < claim.Origin.Native || !boundaryID(string(claim.Action)) || !boundaryID(string(claim.Pawn)) || !boundaryID(string(claim.Claim)) || string(claim.Session) != b.session || !boundaryID(q.PawnSnapshotToken) {
		return out, executor.ErrEvidence
	}
	request := &o.ReleaseOwnedDraftRequest{Identity: boundaryIdentity(claim.Origin), Pawn: &o.EntityPrecondition{EntityId: proto.String(string(claim.Pawn)), ExpectedSnapshotToken: proto.String(q.PawnSnapshotToken)}, ExpectedClaimId: proto.String(string(claim.Claim)), OriginalOwner: &a.Owner{ControllerSessionId: proto.String(string(claim.Session)), PlayerDirection: proto.Uint64(uint64(claim.Origin.Direction))}}
	if err := ctx.Err(); err != nil {
		return out, err
	}
	...
	job := result.Observed
	if job == nil || job.PawnId == nil || job.GetPawnId() != string(claim.Pawn) || job.Drafted == nil || job.GetDrafted() || job.Verified == nil || !job.GetVerified() || job.Issued == nil || job.GetIssued() != issued || job.GetDraftClaimId() != string(claim.Claim) || job.GetDraftOwner() != string(claim.Session) || !boundaryID(job.GetResultingSnapshotToken()) {
		return out, executor.ErrEvidence
	}
...
func validReleaseContext(v *c.ObservationContext, q domain.DraftReleaseRequest) bool {
	return bridge.ValidateContext(v) == nil && proto.Equal(v.Identity, boundaryIdentity(q.Claim.Origin)) && v.NativeGeneration != nil && v.GetNativeGeneration() >= uint64(q.Observed.Native) && v.GetTick() >= int64(q.Tick)
}
