...
func (b *HaulBoundary) ObserveHaul(ctx context.Context, dispatch executor.HaulDispatch, current domain.GenerationSnapshot) (executor.HaulEvidence, error) {
	p := dispatch.Attempt
	out := executor.HaulEvidence{StartedAt: b.clock.Now(), Pawn: dispatch.Admission.Pawn, Thing: dispatch.Admission.Thing, Observation: domain.Observation{Action: p.Action.ID(), Attempt: p.Attempt, Snapshot: current, Effect: domain.EffectUnknown}}
	...
	if job != nil {
		if err = haulJob(job, dispatch); err != nil {
			return out, err
		}
