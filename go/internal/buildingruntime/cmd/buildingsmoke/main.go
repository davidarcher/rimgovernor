// buildingsmoke is a trusted native fixture entrypoint, not a player service.
package main
...
const planID domain.PlanID = "go-building-smoke"
...
func parse(args []string) (options, error) {
	...
	if out.mode == "place" && (!out.execute || !filepath.IsAbs(out.request)) {
		return out, errors.New("place requires --execute and absolute --request")
	}
...
func fixture(data []byte) (domain.PlanSpec, error) {
	if len(data) > 1<<20 {
		return domain.PlanSpec{}, errors.New("fixture exceeds 1 MiB")
	}
	candidate := &p.PlacementCandidate{}
	if err := (protojson.UnmarshalOptions{DiscardUnknown: false, RecursionLimit: 64}).Unmarshal(data, candidate); err != nil {
		return domain.PlanSpec{}, err
	}
	if candidate.DefName == nil || candidate.X == nil || candidate.Z == nil || candidate.Rotation == nil {
		return domain.PlanSpec{}, errors.New("placement presence missing")
	}
	rotation := map[p.Rotation]domain.Rotation{p.Rotation_ROTATION_NORTH: domain.North, p.Rotation_ROTATION_EAST: domain.East, p.Rotation_ROTATION_SOUTH: domain.South, p.Rotation_ROTATION_WEST: domain.West}[candidate.GetRotation()]
	building, err := domain.NewBuilding(candidate.GetDefName(), domain.Cell{X: candidate.GetX(), Z: candidate.GetZ()}, rotation, candidate.GetStuff())
	if err != nil {
		return domain.PlanSpec{}, err
	}
...
func run(args []string, stdout, stderr io.Writer) int {
	...
	fmt.Fprintln(stdout, "building fixture", opts.mode, "passed")
	return 0
}
func perform(opts options, out *report) (err error) {
	...
	if state.Spec.Revision() != 1 || len(state.Spec.Actions()) != 1 || state.Spec.Actions()[0].ID() != actionID || len(state.Progress) != 1 {
		return errors.New("not the fixed building fixture plan")
	}
