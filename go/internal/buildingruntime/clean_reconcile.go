...
func (b *CleanBoundary) ObserveClean(ctx context.Context, dispatch executor.CleanDispatch, current domain.GenerationSnapshot) (executor.CleanEvidence, error) {
	p := dispatch.Attempt
	out := executor.CleanEvidence{StartedAt: b.clock.Now(), Pawn: dispatch.Admission.Pawn, Filth: dispatch.Admission.Filth, Observation: domain.Observation{Action: p.Action.ID(), Attempt: p.Attempt, Snapshot: current, Effect: domain.EffectUnknown}}
	...
	if job != nil {
		if err = cleanJob(job, dispatch); err != nil {
			return out, err
		}
