...
func (b *RangedAttackBoundary) ObserveRanged(ctx context.Context, dispatch executor.RangedDispatch, current domain.GenerationSnapshot) (executor.RangedEvidence, error) {
	p := dispatch.Attempt
	out := executor.RangedEvidence{StartedAt: b.clock.Now(), Pawn: dispatch.Admission.Pawn, Target: dispatch.Admission.Target, Observation: domain.Observation{Action: p.Action.ID(), Attempt: p.Attempt, Snapshot: current, Effect: domain.EffectUnknown}}
	...
	if job != nil {
		if err = rangedJob(job, dispatch); err != nil {
			return out, err
		}
