...
func NewRangedBoundary(native RangedNative, writer RangedWriter, leases LeaseSource, clock executor.Clock, session string) (*RangedAttackBoundary, error) {
...
func (b *RangedAttackBoundary) InspectRanged(ctx context.Context, target executor.Target, claim domain.DraftClaim) (executor.RangedInspection, error) {
	...
	preview, _, err := b.native.PreviewAttack(ctx, boundaryIdentity(current), rangedCommand(string(m.Pawn()), string(m.Target()), pawnToken, targetToken))
	if err != nil {
		return out, err
	}
	...
	facts := policy.RangedDefenseFacts{Snapshot: current, PawnTick: domain.Tick(observed.Context.GetTick()), PreviewTick: domain.Tick(evaluated.Context.GetTick()), NativeCanTry: draftBool(job.CanTry)}
	facts.Emergency, err = policy.NewEmergencySnapshot(current, domain.Tick(emergency.Context.GetTick()), emergency.Facts)
	if err != nil {
		return out, err
	}
	facts.Pawn = policy.RangedPawnFacts{Pawn: m.Pawn(), SnapshotToken: pawnToken, Dead: draftBool(pawn.Dead), Downed: draftBool(pawn.Downed), FreeColonist: draftBool(pawn.FreeColonist), Drafted: draftBool(pawn.Drafted)}
	if pawn.Health != nil {
	...
	facts.Target = policy.RangedTargetFacts{Pawn: m.Target(), SnapshotToken: targetToken, Dead: draftBool(opponent.Dead), Downed: draftBool(opponent.Downed), Hostile: draftBool(opponent.Hostile)}
