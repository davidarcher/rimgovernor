...
func (b *CaptureBoundary) ObserveCapture(ctx context.Context, dispatch executor.CaptureDispatch, current domain.GenerationSnapshot) (executor.CaptureEvidence, error) {
	p := dispatch.Attempt
	out := executor.CaptureEvidence{StartedAt: b.clock.Now(), Capturer: dispatch.Admission.Capturer, Patient: dispatch.Admission.Patient, Observation: domain.Observation{Action: p.Action.ID(), Attempt: p.Attempt, Snapshot: current, Effect: domain.EffectUnknown}}
	...
	if job != nil {
		if err = captureJob(job, dispatch); err != nil {
			return out, err
		}
