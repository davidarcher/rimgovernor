...
func (b *RescueBoundary) ObserveRescue(ctx context.Context, dispatch executor.RescueDispatch, current domain.GenerationSnapshot) (executor.RescueEvidence, error) {
	p := dispatch.Attempt
	out := executor.RescueEvidence{StartedAt: b.clock.Now(), Rescuer: dispatch.Admission.Rescuer, Patient: dispatch.Admission.Patient, Observation: domain.Observation{Action: p.Action.ID(), Attempt: p.Attempt, Snapshot: current, Effect: domain.EffectUnknown}}
	...
	if job != nil {
		if err = rescueJob(job, dispatch); err != nil {
			return out, err
		}
