...
func TestRoutineDrugPolicyUsesDurableWorkAction(t *testing.T) {
	...
	if !workActionStale(w, map[domain.PawnID]domain.WorkAssignment{w.Pawn(): replacement}) {
		t.Fatal("changed policy target retained stale action")
	}
}
