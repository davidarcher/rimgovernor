...
func TestCoordinateAdmitsHigherRankAndReportsLoserWaiting(t *testing.T) {
	...
	for _, order := range [][2]string{{"high", "low"}, {"low", "high"}} {
		...
		outcomes, failures := a.coordinate(context.Background(), map[policy.Resource]int64{"Steel": 25})
		...
		if !outcomes[0].Admitted || outcomes[0].Proposal != "high" || outcomes[0].Plan != "plan-high" || settled["high"] != outcomes[0] {
			t.Fatalf("%v: high must be admitted first: %+v", order, outcomes[0])
		}
		if outcomes[1].Admitted || outcomes[1].Reason != BuildingMethodWaiting || outcomes[1].Waiting != "pawn:builder held by high" || settled["low"] != outcomes[1] {
...
func TestCoordinateQuantityBudgetRefusesSecondClaim(t *testing.T) {
	...
	outcomes, _ := a.coordinate(context.Background(), map[policy.Resource]int64{"Steel": 40})
	...
	if outcomes, _ = b.coordinate(context.Background(), nil); len(again) != 2 || !outcomes[1].Admitted {
...
func TestCoordinateRankOrder(t *testing.T) {
	...
	outcomes, _ := a.coordinate(context.Background(), nil)
...
func TestCoordinateHonoursArbiterClaimsAndSettlesNonProposals(t *testing.T) {
	...
	outcomes, failures := a.coordinate(context.Background(), nil)
