...
func (b *DraftBoundary) historicalClaim(p executor.Placement, receipt *r.Receipt, row *n.PawnState, observed *c.ObservationContext) (domain.Fact[domain.DraftClaim], error) {
	...
	if job == nil || job.GetPawnId() != string(pawn.Pawn()) || job.Drafted == nil || !job.GetDrafted() || job.Verified == nil || !job.GetVerified() || job.Issued == nil || job.GetIssued() != issued || job.GetDraftOwner() != b.session || !boundaryID(job.GetDraftClaimId()) || !boundaryID(job.GetResultingSnapshotToken()) {
	...
	case *n.DraftClaimObservation_Owned:
		owned := state.Owned
		if owned == nil || !boundaryID(owned.GetClaimId()) || owned.Owner == nil || !boundaryID(owned.Owner.GetControllerSessionId()) || owned.Owner.PlayerDirection == nil || owned.Owner.GetPlayerDirection() == 0 || !proto.Equal(owned.PawnSnapshot, row.Pawn.Snapshot) {
			return unknown, executor.ErrHeld
		}
