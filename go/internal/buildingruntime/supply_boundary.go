...
func (b *supplyBoundary) AllowSupply(ctx context.Context, request executor.SupplyDispatch) (executor.Receipt, error) {
	...
	reply, _, err := b.supply.Writer.AllowSupply(ctx, &a.WritePrecondition{Identity: boundaryIdentity(p.Snapshot), Attempt: b.attempt(p), LeaseId: proto.String(lease), ExpectedGeneration: proto.Uint64(uint64(p.Snapshot.Native))}, bridge.SupplyTarget{Supply: supply, Token: request.SnapshotToken})
	if err != nil {
		return out, err
