...
func (b *TendBoundary) ObserveTend(ctx context.Context, dispatch executor.TendDispatch, current domain.GenerationSnapshot) (executor.TendEvidence, error) {
	p := dispatch.Attempt
	out := executor.TendEvidence{StartedAt: b.clock.Now(), Doctor: dispatch.Admission.Doctor, Patient: dispatch.Admission.Patient, Observation: domain.Observation{Action: p.Action.ID(), Attempt: p.Attempt, Snapshot: current, Effect: domain.EffectUnknown}}
	...
	if job != nil {
		if err = tendJob(job, dispatch); err != nil {
			return out, err
		}
