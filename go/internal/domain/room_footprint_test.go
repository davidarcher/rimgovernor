...
func TestRectangleFootprintMatchesRoomShell(t *testing.T) {
	for _, entrance := range []Rotation{North, East, South, West} {
		shell, err := NewRoomShell(RoomBounds{X: 3, Z: 5, Width: 9, Height: 7}, "Wall", "Door", "WoodLog", entrance, ShelterRoom)
		if err != nil {
			t.Fatal(err)
		}
		f, err := RectangleFootprint(shell.Bounds(), entrance)
		if err != nil {
			t.Fatal(err)
		}
		if f.Door() != shell.Door() {
			t.Fatalf("%s door %v, shell door %v", entrance, f.Door(), shell.Door())
		}
		if len(f.Interior()) != 7*5 || len(f.Walls()) != 2*9+2*5 {
			t.Fatalf("interior %d walls %d", len(f.Interior()), len(f.Walls()))
		}
		if f.Bounds() != shell.Bounds() {
		...
		want := shell.Placements()
		if len(got) != len(want) {
			t.Fatalf("placements %d want %d", len(got), len(want))
		}
		for i := range got {
			if got[i] != want[i] {
