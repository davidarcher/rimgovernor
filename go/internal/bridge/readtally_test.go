...
func TestReadTallyCountsPerStepAndSummarizes(t *testing.T) {
	...
	steps := summary.Steps
	if steps.Steps != 2 || steps.Reads != 6 || steps.MaxReads != 4 || steps.CacheHits != 4 || steps.ParentHits != 6 || steps.Tools["rimgovernor/lifecycle_read_identity"] != 4 || steps.Tools["games_status"] != 2 || steps.Windows != 1 || steps.WindowTicks != 2700 || steps.MaxWindowTicks != 2700 {
		t.Fatalf("step summary: %+v", steps)
	}
	var report bytes.Buffer
	WritePhaseReport(&report, summary)
	if text := report.String(); !strings.Contains(text, "steps: 2, reads/step mean 3.0 max 4, cache hits/step 2.0, parent hits/step 3.0, window ticks mean 2700 max 2700 over 1 sized steps") || !strings.Contains(text, "rimgovernor/lifecycle_read_identity") {
