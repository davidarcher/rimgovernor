...
type Snapshot struct {
	...
	// TradeableTraders are exact already-observed trader identities a
	// trade_economy command may open a session with: native listed them and
	// reported canTradeNow. It is the Go form of Python's own list_traders
	// discovery, which refuses a trader that is absent, ambiguous or not
	// currently tradeable. Whether the trader is still tradeable at dispatch,
	// and whether the negotiator can actually reach it, are established at
	// inspection, not here.
	TradeableTraders []domain.SettlementID
}
...
type Proposal struct {
	...
	EvaluateWorld bool
	// TradeEconomy carries trade_economy's resolved trader, negotiator,
	// economic policy and net spend ceiling: Plan is then zero and
	// TradeEconomy.Set() reports true, exclusively of every other outcome
	// above.
	//
	// It carries no Action, for a reason unique in this family: a TradeEconomy
	// request is not one native call but a negotiation of four, and which
	// lines its middle phase must carry cannot be known until its first phase
	// has actually run against native and produced a live session to read a
	// sheet from. The interpreter therefore proposes the whole request, and
	// store.SubmitTradeEconomy commits only its first phase (a TradeOpen);
	// buildingruntime.TradeEconomy commits each later phase as its own
	// one-action plan once the phase before it is observed complete.
	//
	// The trader is bounded against Facts.TradeableTraders, the negotiator
	// against Facts.Pawns and every target item against
	// Facts.ResourceDefinitions -- the same definition list the two
	// resource-policy commands bound against, because an economic target names
	// the same kind of thing.
	TradeEconomy domain.TradeEconomyRequest
	Budget       Budget
}
...
func (i *Interpreter) Interpret(ctx context.Context, input Input) (Proposal, error) {
	...
	// An economic negotiation carries no actions here for a reason no other
	// command shares: the lines its middle phase must stage are native line
	// ids that do not exist until its first phase has run. Only the three
	// things the model actually named are bounded against supplied facts; see
	// Proposal.TradeEconomy.
	if result.Command == "trade_economy" {
		request, err := i.tradeEconomy(input, result)
		if err != nil {
			return proposal, err
		}
		proposal.TradeEconomy = request
		proposal.Generation = input.Current
		return proposal, nil
	}
...
// tradeEconomy bounds one economic negotiation request against supplied facts
// and builds the domain value the submission consumes. Each of the three
// bindable things is checked against the list that actually establishes it:
// the trader against the traders native reported as currently tradeable, the
// negotiator against observed pawns, and every target item against the same
// resource-definition list the resource-policy commands bound against. Nothing
// about whether the deal is worth taking is decided here -- that is
// policy.SelectTrade's job, and it cannot run until a session exists.
func (i *Interpreter) tradeEconomy(input Input, result modelCommand) (domain.TradeEconomyRequest, error) {
	var zero domain.TradeEconomyRequest
	trader := *result.Trader
	tradeable := false
	for _, known := range input.Facts.TradeableTraders {
		if string(known) == trader {
			tradeable = true
			break
		}
	}
	if !tradeable {
		return zero, fail(UnknownFacts, "trader absent from supplied facts")
	}
	if !i.knownPawn(input, *result.TradeNegotiator) {
		return zero, fail(UnknownFacts, "negotiator absent from supplied facts")
	}
	request := domain.TradeEconomyRequest{
		Trader: domain.SettlementID(trader), Negotiator: domain.PawnID(*result.TradeNegotiator),
		MaxSilverSpend: int64(*result.MaxSilverSpend),
		Policy:         domain.TradeEconomicPolicy{SilverReserve: int64(*result.TradeSilverReserve)},
	}
	for _, target := range result.TradeTargets {
		if !i.knownResource(input, target.Item) {
			return zero, fail(UnknownFacts, "economic target absent from supplied facts")
		}
		converted := domain.TradeTarget{Item: target.Item, Stock: int64(*target.Stock)}
		if target.MaxBuy != nil {
			converted.MaxBuy = int64(*target.MaxBuy)
		}
		if target.MaxSell != nil {
			converted.MaxSell = int64(*target.MaxSell)
		}
		if target.MaxBuyPrice != nil {
			converted.MaxBuyPrice = *target.MaxBuyPrice
		}
		if target.MinSellPrice != nil {
			converted.MinSellPrice = *target.MinSellPrice
		}
		request.Policy.Targets = append(request.Policy.Targets, converted)
	}
	if request.Validate() != nil {
		return zero, fail(InvalidCommand, "economic policy out of supported range")
	}
	return request, nil
}
