...
func TestBuildingRequestRejectMalformed(t *testing.T) {
	decode := map[string]func(io.Reader) error{"submission": func(r io.Reader) error { _, e := decodeBuildingSubmission(r); return e }, "manual": func(r io.Reader) error { _, e := decodeBuildingManual(r); return e }}
	for name, raw := range map[string]string{"submission": submissionJSON, "manual": manualJSON} {
