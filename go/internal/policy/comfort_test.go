...
func TestComfortCapacityAndObservedUseAreIndependent(t *testing.T) {
	...
	check := func(want ComfortMethod, fraction float64) ComfortReview {
		t.Helper()
		r, err := ReviewComfort(domain.Known(v), h, 10)
		if err != nil {
			t.Fatal(err)
		}
		method, err := SelectComfortMethod(v, r)
		if err != nil || method != want || r.Deficit() != domain.Known(fraction) {
	...
	if r.Recovered() != domain.Known(false) {
		t.Fatal("construction certified use", r)
	}
	v.Dining[0].Users = []PawnID{"a"}
	check(ComfortWait, .5)
	v.Dining[0].Users = nil
	v.Recreation[0].Users = []PawnID{"b"}
	r = check(ComfortNoMethod, 0)
	if r.Recovered() != domain.Known(true) || r.History.Dining.Facility != "chair" || r.History.Recreation.Facility != "hoop" {
		t.Fatal(r)
	}
...
func TestComfortPreservesAccessAndUnknownProof(t *testing.T) {
	...
	if err != nil || r.History != h || r.Recovered() != domain.Unknown[bool]() {
		t.Fatal(r, err)
	}
	...
	if err != nil || m != ComfortAccessBlocked || r.MissingDining != 1 || r.Recovered() != domain.Known(false) {
		t.Fatal(r, m, err)
	}
	v = comfortFacilities()
	v.Dining[0].ID = "replacement"
	r, err = ReviewComfort(domain.Known(v), h, 9)
	if err != nil || r.Dining != ComfortUseNeeded {
		t.Fatal("old furniture use transferred", r, err)
	}
	v.Dining[0].Users = []PawnID{"a"}
	v.Dining[0].AccessibleTo = []PawnID{"b"}
	r, err = ReviewComfort(domain.Known(v), h, 9)
	if err != nil || r.History.Dining.Facility != "chair" {
		t.Fatal("inaccessible use accepted", r, err)
	}
	if _, err = ReviewComfort(domain.Known(v), h, 6); err == nil {
		t.Fatal("future use accepted")
	}
	if _, err = ReviewComfort(domain.Known(v), ComfortHistory{Dining: ComfortUse{Tick: 1}}, 9); err == nil {
		t.Fatal("unbound use accepted")
	}
}
...
func TestComfortRejectsIncompleteIdentitySets(t *testing.T) {
	for _, mutate := range []func(*ComfortObservation){
		...
		func(v *ComfortObservation) {
			v.Surfaces[0].Adjacent = append(v.Surfaces[0].Adjacent, v.Surfaces[0].Adjacent[0])
		},
	} {
		v := comfortFacilities()
		mutate(&v)
		if _, err := ReviewComfort(domain.Known(v), ComfortHistory{}, 1); err == nil {
			t.Fatal("invalid comfort census accepted", v)
		}
	}
}
