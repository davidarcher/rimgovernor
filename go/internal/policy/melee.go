...
// EvaluateMeleeDefense admits one observed opponent under an existing exact
// draft claim. Admission neither grants authority nor proves combat completion.
func EvaluateMeleeDefense(r MeleeDefenseRequest) DraftDecision {
	refuse := func(reason Reason) DraftDecision {
		return DraftDecision{Refused: []Refusal{{Action: r.Action.ID(), Reason: reason}}}
	}
	m, ok := r.Action.MeleeAttack()
	canonical, err := domain.NewMeleeAttackAction(r.Action.ID(), m)
	...
	equipment, _ := f.Pawn.EquipmentKnown.Value()
	if !equipment {
