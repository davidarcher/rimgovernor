...
func TestQuestAcceptAdmissionWithoutAccepterOrChoice(t *testing.T) {
	...
	// A step-cached census behind the minimum admits while the preview is
	// at or past it (#250: the second inspection of a live step).
	r.MinimumTick, r.Facts.QuestTick, r.Facts.PreviewTick = 13, 12, 13
	if d := EvaluateQuestAccept(r); !d.Admitted || len(d.Refused) != 0 {
		t.Fatal(d)
	}
}
...
func TestQuestAcceptDefenseHolds(t *testing.T) {
	cases := []struct {
		...
		{"minimum", func(r *QuestAcceptRequest) { r.MinimumTick = 14 }},
