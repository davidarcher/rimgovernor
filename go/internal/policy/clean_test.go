...
func cleanRequest(t *testing.T) CleanRequest {
	...
	pawn := CleanPawnFacts{Pawn: "cleaner", SnapshotToken: "pawn-cas", Dead: domain.Known(false), Downed: domain.Known(false), Drafted: domain.Known(false), MentalState: domain.Known(false), PlayerForced: domain.Known(false), QueuedJobs: domain.Known(uint32(0)), ExistingJobDef: domain.Known("")}
...
func TestCleanDefenseHolds(t *testing.T) {
	cases := []struct {
		...
		{"pawn player forced", func(r *CleanRequest) { r.Facts.Pawn.PlayerForced = domain.Known(true) }},
		{"pawn queued", func(r *CleanRequest) { r.Facts.Pawn.QueuedJobs = domain.Known(uint32(1)) }},
		{"pawn unknown queue", func(r *CleanRequest) { r.Facts.Pawn.QueuedJobs = domain.Unknown[uint32]() }},
