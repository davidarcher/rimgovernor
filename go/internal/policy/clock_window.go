...
func EvaluateClockWindow(f ClockWindowFacts, limits ClockWindowLimits) ClockWindowDecision {
	...
	// admits from facts observed within the tolerance of this tick and
	// never from facts an earlier window has outrun. Unknown means no
	// planner has observed yet: the work then comes from the journal alone.
	if planned, known := f.FactsTick.Value(); known && (planned > f.Tick || f.Tick-planned > f.FactsTolerance) {
