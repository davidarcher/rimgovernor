...
func TestCaptureDefenseHolds(t *testing.T) {
	cases := []struct {
		...
		{"direction", func(r *CaptureRequest) { r.Current.Direction++ }},
		...
		{"capturer player forced", func(r *CaptureRequest) { r.Facts.Capturer.PlayerForced = domain.Known(true) }},
		{"capturer queued", func(r *CaptureRequest) { r.Facts.Capturer.QueuedJobs = domain.Known(uint32(1)) }},
		{"capturer unknown queue", func(r *CaptureRequest) { r.Facts.Capturer.QueuedJobs = domain.Unknown[uint32]() }},
