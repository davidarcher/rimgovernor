...
func equipRequest(t *testing.T) EquipRequest {
	...
	pawn := EquipPawnFacts{Pawn: "unarmed", SnapshotToken: "pawn-cas", Dead: domain.Known(false), Downed: domain.Known(false), Drafted: domain.Known(false), MentalState: domain.Known(false), PlayerForced: domain.Known(false), QueuedJobs: domain.Known(uint32(0)), ExistingJobDef: domain.Known("")}
...
func TestEquipDefenseHolds(t *testing.T) {
	cases := []struct {
		...
		{"pawn player forced", func(r *EquipRequest) { r.Facts.Pawn.PlayerForced = domain.Known(true) }},
		{"pawn queued", func(r *EquipRequest) { r.Facts.Pawn.QueuedJobs = domain.Known(uint32(1)) }},
		{"pawn unknown queue", func(r *EquipRequest) { r.Facts.Pawn.QueuedJobs = domain.Unknown[uint32]() }},
