...
func TestDefenseRearmTurretsCountsEmptyBarrelsAndOrdersOneRefuel(t *testing.T) {
	...
	got := DefenseRearmTurrets(turrets, workers, domain.Known(map[Resource]int64{"Steel": 120}))
	...
	got = DefenseRearmTurrets(turrets, []WorkPawn{rearmWorker("nohaul", true, 0, true), rearmWorker("idle", true, 0, false)}, domain.Known(map[Resource]int64{"Steel": 1}))
	if len(got.Rearm) != 1 || got.Rearm[0].Pawn != "idle" {
		t.Fatalf("%+v", got)
	}
	if got = DefenseRearmTurrets(turrets, []WorkPawn{rearmWorker("nohaul", true, 0, true)}, domain.Known(map[Resource]int64{"Steel": 1})); len(got.Rearm) != 0 || len(got.Empty) != 2 {
		t.Fatalf("%+v", got)
	}
}
...
func TestDefenseRearmTurretsReportsShortageAndUnknowns(t *testing.T) {
	...
	got := DefenseRearmTurrets([]DefenseTurretFacts{empty, empty}, workers, domain.Known(map[Resource]int64{"WoodLog": 500}))
	...
	got = DefenseRearmTurrets([]DefenseTurretFacts{empty}, workers, domain.Unknown[map[Resource]int64]())
	...
	got = DefenseRearmTurrets([]DefenseTurretFacts{full, unknown, levels}, workers, domain.Known(map[Resource]int64{}))
	...
	anonymous := empty
	anonymous.ID = ""
	if got = DefenseRearmTurrets([]DefenseTurretFacts{anonymous}, workers, domain.Known(map[Resource]int64{"Steel": 9})); len(got.Rearm) != 0 || len(got.Empty) != 1 {
		t.Fatalf("%+v", got)
	}
}
