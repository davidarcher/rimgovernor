...
func (n *workEnvironment) AssignWork(_ context.Context, request WorkDispatch) (Receipt, error) {
	n.allowed++
	if request.SnapshotToken != "work-token" {
...
func (n *workEnvironment) ObserveWork(_ context.Context, p Placement, current domain.GenerationSnapshot) (WorkEvidence, error) {
	n.observed++
	work, _ := p.Action.WorkAssignment()
	if n.foreign {
		work, _ = domain.NewWorkAssignment("foreign", work.BeforeToken(), work.Manual(), work.Settings())
	}
	effect := n.effect
	if effect == "" {
...
func workFixture(t *testing.T) (*fixture, *workEnvironment) {
	t.Helper()
	f := newFixture(t)
	work, _ := domain.NewWorkAssignment("pawn", "work-token", true, []domain.WorkSetting{{Definition: "Cooking", Priority: 1}})
	action, _ := domain.NewWorkAssignmentAction("allow-1", work)
	plan, _ := domain.NewPlan("supplies", 1, []domain.Action{action})
	if err := f.store.CreatePlan(context.Background(), plan); err != nil {
		t.Fatal(err)
	}
	n := &workEnvironment{environment: f.env}
...
func TestWorkUnknownReplyReopensAndObservesAfterManual(t *testing.T) {
	f, n := workFixture(t)
	n.uncertain = true
	result, err := f.run()
	...
	defer f.store.Close()
	f.executor.journal, f.executor.workJournal = f.store, f.store
	...
	if err != nil || len(state.WorkAdmissions) != 1 || state.Spec.Actions()[0] != f.action {
		t.Fatal(state, err)
	}
}
func TestWorkEmergencyAndDirectionChangesBlockDispatch(t *testing.T) {
...
func TestWorkRejectsForeignCompletionAndAbsence(t *testing.T) {
	f, n := workFixture(t)
	if _, err := f.run(); err != nil {
		t.Fatal(err)
	}
...
func TestWorkTypedPreparationCannotBeBypassed(t *testing.T) {
	f, _ := workFixture(t)
