...
func (e *Executor) runQuestAccept(ctx context.Context, action domain.Action, p domain.Progress, authority Authority, generation context.Context) (Result, error) {
	...
	v := p.View()
	if v.Unresolved {
		return e.reconcileQuestAccept(ctx, result, generation)
	}
	switch v.Stage {
	...
	minimum := v.Tick
	var inspection QuestAcceptInspection
	for range 2 {
		if err := e.guard(ctx, expected, generation); err != nil {
			return result, err
		}
		var err error
		inspection, err = e.questAccept.InspectQuestAccept(ctx, Target{action, expected})
		if err != nil {
			return result, err
		}
		if err = e.guard(ctx, expected, generation); err != nil {
		...
		minimum = max(minimum, facts.QuestTick)
		...
		next, err := e.questAcceptJournal.PrepareQuestAccept(ctx, v.Plan, v.Action, admission)
		if err != nil {
			return result, err
		}
	...
	admission := store.QuestAcceptAdmission{Snapshot: expected, Tick: inspection.Facts.PreviewTick, Quest: accept.Quest(), AccepterPawn: accept.AccepterPawn(), RewardChoice: accept.RewardChoice(), QuestSnapshotToken: inspection.Facts.Quest.SnapshotToken}
	if err = e.guard(ctx, expected, generation); err != nil {
	...
	receipt, err := e.questAccept.WriteQuestAccept(ctx, QuestAcceptDispatch{attempt, admission})
	kind := receipt.Kind
	if err != nil {
...
func (e *Executor) reconcileQuestAccept(ctx context.Context, result Result, generation context.Context) (Result, error) {
	p := result.Progress
	v := p.View()
	...
	var admission store.QuestAcceptAdmission
	found := false
	...
	evidence, err := e.questAccept.ObserveQuestAccept(ctx, QuestAcceptDispatch{Placement{p.Action(), v.Attempt, v.Snapshot, v.Tick}, admission}, current)
	if err != nil {
		return result, err
	}
	if ctx.Err() != nil {
	...
	accept, _ := p.Action().QuestAccept()
	o := evidence.Observation
	if o.Action != v.Action || o.Attempt != v.Attempt || o.Snapshot != current || evidence.Quest != accept.Quest() || !e.fresh(evidence.StartedAt, evidence.ObservedAt) {
		return result, ErrEvidence
	}
	switch o.Effect {
