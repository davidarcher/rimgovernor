...
func (e *Executor) runDialogAnswer(ctx context.Context, action domain.Action, p domain.Progress, authority Authority, generation context.Context) (Result, error) {
	result := Result{Progress: p}
	value, ok := action.DialogAnswer()
	...
	if v.Unresolved {
		...
		evidence, err := e.dialog.ObserveDialogAnswer(ctx, Placement{action, v.Attempt, v.Snapshot, v.Tick}, current)
		if err != nil {
			return result, err
		}
		if ctx.Err() != nil {
	...
	var inspection DialogAnswerInspection
	for range 2 {
		if err := e.guard(ctx, expected, generation); err != nil {
			return result, err
		}
		var err error
		inspection, err = e.dialog.InspectDialogAnswer(ctx, Target{action, expected})
		if err != nil {
			return result, err
		}
		if err = e.guard(ctx, expected, generation); err != nil {
		...
		decision := policy.EvaluateDialogAnswer(policy.DialogAnswerRequest{Action: action, Progress: result.Progress, Current: expected, MinimumTick: v.Tick, Facts: inspection.Facts})
	...
	receipt, err := e.dialog.AnswerDialog(ctx, DialogAnswerDispatch{attempt, value.WindowID(), value.OptionIndex(), value.OptionLabel()})
	kind := receipt.Kind
	if err != nil {
