...
func NewWithRanged(journal RangedJournal, building Boundary, draft DraftBoundary, ranged RangedBoundary, clock Clock, limits Limits, routine ...RoutineScope) (*Executor, error) {
	if ranged == nil {
...
func rangedProgress(state store.PlanState, id domain.ActionID) (domain.Progress, bool) {
...
func (e *Executor) runRangedAttack(ctx context.Context, action domain.Action, progress domain.Progress, authority Authority, generation context.Context) (Result, error) {
	...
	v := progress.View()
	if v.Unresolved {
		return e.reconcileRangedAttack(ctx, result, generation)
	}
	switch v.Stage {
	...
	for range 2 {
		...
		latest, exists := rangedProgress(state, v.Action)
		if !exists || latest.Action() != action {
			return result, ErrEvidence
		}
		result.Progress = latest
		prerequisite, exists := rangedProgress(state, m.DraftAction())
		if !exists {
			return result, ErrEvidence
		}
		...
		inspection, err = e.ranged.InspectRanged(ctx, Target{action, expected}, claim)
		if err != nil {
			return result, err
		}
		if err = e.guard(ctx, expected, generation); err != nil {
...
func (e *Executor) reconcileRangedAttack(ctx context.Context, result Result, generation context.Context) (Result, error) {
	p := result.Progress
	v := p.View()
	...
	evidence, err := e.ranged.ObserveRanged(ctx, RangedDispatch{draftAttempt(p.Action(), p), admission}, current)
	if err != nil {
		return result, err
	}
	if ctx.Err() != nil {
