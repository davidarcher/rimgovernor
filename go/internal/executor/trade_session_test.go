...
// TestResolveTradeDependencyIgnoresEscortEdges is the routine accept plan's
// shape (#234): the accept depends on the escort goto for ordering, and only
// the cross-plan reference names the open session. A non-trade sibling never
// reads as the session.
func TestResolveTradeDependencyIgnoresEscortEdges(t *testing.T) {
	f := tradeResolutionFixture(t)
	ctx := context.Background()
	owned, err := domain.NewOwnedDraft("negotiator-1")
	if err != nil {
		t.Fatal(err)
	}
	draft, err := domain.NewOwnedDraftAction("escort-draft", owned)
	if err != nil {
		t.Fatal(err)
	}
	tradePlan(t, f.store, "open-plan", []domain.Action{tradeOpenAction(t, "open")})
	tradePlan(t, f.store, "accept-plan", []domain.Action{draft, tradeLinesAction(t, "accept")},
		domain.ActionDependency{Action: "accept", Requires: "escort-draft"})
	if err := f.store.RecordTradeSessionReference(ctx, "accept", "open"); err != nil {
		t.Fatal(err)
	}
	session := store.TradeSession{SessionID: "mine", SessionToken: "mine-token"}
	if err := f.store.RecordTradeSession(ctx, "open", session); err != nil {
		t.Fatal(err)
	}
	got, err := f.executor.resolveTradeDependency(ctx, "accept-plan", "accept", domain.TradeSetLines)
	if err != nil || got.OpenAction != "open" || got.Session != session {
		t.Fatalf("got %+v err %v, want the cross-plan reference past the escort edge", got, err)
	}
}
