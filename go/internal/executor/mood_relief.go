...
func (e *Executor) runMoodRelief(ctx context.Context, action domain.Action, p domain.Progress, authority Authority, generation context.Context) (Result, error) {
	result := Result{Progress: p}
	relief, ok := action.MoodRelief()
	if !ok || p.Action() != action {
		return result, ErrEvidence
	}
	v := p.View()
	if v.Unresolved {
		return e.reconcileMoodRelief(ctx, result, generation)
	}
	switch v.Stage {
	...
	minimum := v.Tick
	var inspection MoodReliefInspection
	for range 2 {
		if err := e.guard(ctx, expected, generation); err != nil {
			return result, err
		}
		var err error
		inspection, err = e.moodRelief.InspectMoodRelief(ctx, Target{action, expected})
		if err != nil {
			return result, err
		}
		if err = e.guard(ctx, expected, generation); err != nil {
		...
		decision := policy.EvaluateMoodRelief(policy.MoodReliefDispatchRequest{Action: action, Progress: result.Progress, Current: expected, MinimumTick: minimum, Facts: facts})
		...
		admission := store.MoodReliefAdmission{Snapshot: expected, Tick: facts.PreviewTick, Pawn: relief.Pawn(), Need: relief.Need(), ExpectedJob: relief.ExpectedJob(), ExpectedScheduleDef: relief.ExpectedScheduleDef(), PawnSnapshotToken: facts.Pawn.SnapshotToken}
		next, err := e.moodReliefJournal.PrepareMoodRelief(ctx, v.Plan, v.Action, admission)
		if err != nil {
			return result, err
		}
	...
	admission := store.MoodReliefAdmission{Snapshot: expected, Tick: inspection.Facts.PreviewTick, Pawn: relief.Pawn(), Need: relief.Need(), ExpectedJob: relief.ExpectedJob(), ExpectedScheduleDef: relief.ExpectedScheduleDef(), PawnSnapshotToken: inspection.Facts.Pawn.SnapshotToken}
	if err = e.guard(ctx, expected, generation); err != nil {
	...
	receipt, err := e.moodRelief.ManageMoodRelief(ctx, MoodReliefDispatch{attempt, admission})
	kind := receipt.Kind
	if err != nil {
...
func (e *Executor) reconcileMoodRelief(ctx context.Context, result Result, generation context.Context) (Result, error) {
	p := result.Progress
	v := p.View()
	...
	var admission store.MoodReliefAdmission
	found := false
	for _, record := range state.MoodReliefAdmissions {
	...
	evidence, err := e.moodRelief.ObserveMoodRelief(ctx, MoodReliefDispatch{Placement{p.Action(), v.Attempt, v.Snapshot, v.Tick}, admission}, current)
	if err != nil {
		return result, err
	}
	if ctx.Err() != nil {
	...
	relief, _ := p.Action().MoodRelief()
	o := evidence.Observation
	if o.Action != v.Action || o.Attempt != v.Attempt || o.Snapshot != current || evidence.Pawn != relief.Pawn() || !e.fresh(evidence.StartedAt, evidence.ObservedAt) {
		return result, ErrEvidence
	}
	switch o.Effect {
