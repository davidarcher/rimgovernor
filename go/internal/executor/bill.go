...
type BillJournal interface {
	Journal
	PrepareBill(context.Context, domain.PlanID, domain.ActionID, store.BillAdmission) (domain.Progress, error)
}
type BillInspection struct {
...
type BillBoundary interface {
	InspectBill(context.Context, Target) (BillInspection, error)
	AddBill(context.Context, BillDispatch) (Receipt, error)
...
func (e *Executor) runBill(ctx context.Context, action domain.Action, p domain.Progress, authority Authority, generation context.Context) (Result, error) {
	...
	if v.Unresolved {
		...
		evidence, err := e.bill.ObserveBill(ctx, Placement{action, v.Attempt, v.Snapshot, v.Tick}, current)
		if err != nil {
			return result, err
		}
		if ctx.Err() != nil {
		...
		case domain.EffectCompleted:
			...
			if !evidence.Complete || evidence.Bill != bill || !known || !allowed || evidence.Iterations == 0 || !evidence.OutputComplete || !evidence.OutputObserved || evidence.OutputCount == 0 {
				return result, ErrEvidence
			}
	...
	var inspection BillInspection
	for range 2 {
		if err := e.guard(ctx, expected, generation); err != nil {
			return result, err
		}
		var err error
		inspection, err = e.bill.InspectBill(ctx, Target{action, expected})
		if err != nil {
			return result, err
		}
		if err = e.guard(ctx, expected, generation); err != nil {
