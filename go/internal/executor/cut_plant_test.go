...
type cutPlantEnvironment struct {
	*environment
...
func (n *cutPlantEnvironment) InspectCutPlant(_ context.Context, target Target) (CutPlantInspection, error) {
	n.inspected++
	if n.onInspect != nil {
		n.onInspect()
	}
...
func (n *cutPlantEnvironment) DesignateCutPlant(_ context.Context, request CutPlantDispatch) (Receipt, error) {
	n.allowed++
	if request.SnapshotToken != "cut-token" {
...
func (n *cutPlantEnvironment) ObserveCutPlant(_ context.Context, p Placement, current domain.GenerationSnapshot) (CutPlantEvidence, error) {
	n.observed++
	plant, _ := p.Action.CutPlant()
	if n.foreign {
		plant, _ = domain.NewCutPlant("foreign", plant.Definition(), plant.Cell())
	}
	effect := n.effect
	if effect == "" {
...
func TestCutPlantUnknownReplyReopensAndObservesAfterManual(t *testing.T) {
	f, n := cutPlantFixture(t)
	n.uncertain = true
	result, err := f.run()
	...
	defer f.store.Close()
	f.executor.journal, f.executor.cutPlantJournal = f.store, f.store
	...
	if err != nil || len(state.CutPlantAdmissions) != 1 || state.Spec.Actions()[0] != f.action {
		t.Fatal(state, err)
	}
}
func TestCutPlantEmergencyAndDirectionChangesBlockDispatch(t *testing.T) {
...
func TestCutPlantAbsentBeforeDispatchCancels(t *testing.T) {
	f, n := cutPlantFixture(t)
	n.gone = true
...
func TestCutPlantRejectsForeignCompletionAndAbsence(t *testing.T) {
	f, n := cutPlantFixture(t)
	if _, err := f.run(); err != nil {
		t.Fatal(err)
	}
...
func TestCutPlantUnadmittedAttemptRetries(t *testing.T) {
	f, n := cutPlantFixture(t)
	n.uncertain = true
...
func TestCutPlantTypedPreparationCannotBeBypassed(t *testing.T) {
	f, _ := cutPlantFixture(t)
	...
	wrong := store.CutPlantAdmission{Snapshot: f.authority.Snapshot, Tick: 100, Thing: "other", SnapshotToken: "token"}
	if _, err := f.store.PrepareCutPlant(context.Background(), f.plan.ID(), f.action.ID(), wrong); err == nil {
		t.Fatal("foreign item admitted")
	}
}

// A designated plant still standing is pending work, not a settled attempt;
// the executor keeps observing until the plant is gone.
func TestCutPlantPendingStaysOpenUntilThePlantIsGone(t *testing.T) {
	f, n := cutPlantFixture(t)
	if _, err := f.run(); err != nil {
		t.Fatal(err)
	}
