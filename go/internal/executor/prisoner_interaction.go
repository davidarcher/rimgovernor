...
func (e *Executor) runPrisonerInteraction(ctx context.Context, action domain.Action, p domain.Progress, authority Authority, generation context.Context) (Result, error) {
	result := Result{Progress: p}
	interaction, ok := action.PrisonerInteraction()
	if !ok || p.Action() != action {
		return result, ErrEvidence
	}
	v := p.View()
	if v.Unresolved {
		return e.reconcilePrisonerInteraction(ctx, result, generation)
	}
	switch v.Stage {
	...
	minimum := v.Tick
	var inspection PrisonerInteractionInspection
	for range 2 {
		if err := e.guard(ctx, expected, generation); err != nil {
			return result, err
		}
		var err error
		inspection, err = e.prisonerInteraction.InspectPrisonerInteraction(ctx, Target{action, expected})
		if err != nil {
			return result, err
		}
		if err = e.guard(ctx, expected, generation); err != nil {
		...
		next, err := e.prisonerInteractionJournal.PreparePrisonerInteraction(ctx, v.Plan, v.Action, admission)
		if err != nil {
			return result, err
		}
	...
	admission := store.PrisonerInteractionAdmission{Snapshot: expected, Tick: inspection.Facts.PreviewTick, Pawn: interaction.Pawn(), Interaction: interaction.Interaction(), PawnSnapshotToken: inspection.Facts.Pawn.SnapshotToken}
	if err = e.guard(ctx, expected, generation); err != nil {
	...
	receipt, err := e.prisonerInteraction.WritePrisonerInteraction(ctx, PrisonerInteractionDispatch{attempt, admission})
	kind := receipt.Kind
	if err != nil {
...
func (e *Executor) reconcilePrisonerInteraction(ctx context.Context, result Result, generation context.Context) (Result, error) {
	p := result.Progress
	v := p.View()
	...
	var admission store.PrisonerInteractionAdmission
	found := false
	...
	evidence, err := e.prisonerInteraction.ObservePrisonerInteraction(ctx, PrisonerInteractionDispatch{Placement{p.Action(), v.Attempt, v.Snapshot, v.Tick}, admission}, current)
	if err != nil {
		return result, err
	}
	if ctx.Err() != nil {
	...
	interaction, _ := p.Action().PrisonerInteraction()
	o := evidence.Observation
	if o.Action != v.Action || o.Attempt != v.Attempt || o.Snapshot != current || evidence.Pawn != interaction.Pawn() || !e.fresh(evidence.StartedAt, evidence.ObservedAt) {
		return result, ErrEvidence
	}
	switch o.Effect {
