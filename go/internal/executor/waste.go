...
func (e *Executor) runWaste(ctx context.Context, action domain.Action, p domain.Progress, authority Authority, generation context.Context) (Result, error) {
	...
	v := p.View()
	if v.Unresolved {
		return e.reconcileWaste(ctx, result, generation)
	}
	switch v.Stage {
	...
	minimum := v.Tick
	var inspection WasteInspection
	for range 2 {
		if err := e.guard(ctx, expected, generation); err != nil {
			return result, err
		}
		var err error
		inspection, err = e.waste.InspectWaste(ctx, Target{action, expected})
		if err != nil {
			return result, err
		}
		if err = e.guard(ctx, expected, generation); err != nil {
		...
		decision := policy.EvaluateWaste(policy.WasteDispatchRequest{Action: action, Progress: result.Progress, Current: expected, MinimumTick: minimum, Facts: facts})
		...
		admission := store.WasteAdmission{Snapshot: expected, Tick: facts.PreviewTick, Pawn: waste.Pawn(), Target: waste.Target(), Cell: waste.Cell(), PawnSnapshotToken: facts.Pawn.SnapshotToken, TargetSnapshotToken: facts.Item.SnapshotToken}
		next, err := e.wasteJournal.PrepareWaste(ctx, v.Plan, v.Action, admission)
		if err != nil {
			return result, err
		}
	...
	admission := store.WasteAdmission{Snapshot: expected, Tick: inspection.Facts.PreviewTick, Pawn: waste.Pawn(), Target: waste.Target(), Cell: waste.Cell(), PawnSnapshotToken: inspection.Facts.Pawn.SnapshotToken, TargetSnapshotToken: inspection.Facts.Item.SnapshotToken}
	if err = e.guard(ctx, expected, generation); err != nil {
	...
	receipt, err := e.waste.ManageWaste(ctx, WasteDispatch{attempt, admission})
	kind := receipt.Kind
	if err != nil {
...
func (e *Executor) reconcileWaste(ctx context.Context, result Result, generation context.Context) (Result, error) {
	p := result.Progress
	v := p.View()
	...
	var admission store.WasteAdmission
	found := false
	...
	evidence, err := e.waste.ObserveWaste(ctx, WasteDispatch{Placement{p.Action(), v.Attempt, v.Snapshot, v.Tick}, admission}, current)
	if err != nil {
		return result, err
	}
	if ctx.Err() != nil {
	...
	waste, _ := p.Action().Waste()
	o := evidence.Observation
	if o.Action != v.Action || o.Attempt != v.Attempt || o.Snapshot != current || evidence.Pawn != waste.Pawn() || evidence.Target != waste.Target() || !e.fresh(evidence.StartedAt, evidence.ObservedAt) {
		return result, ErrEvidence
	}
	switch o.Effect {
