...
func (e *Executor) runApparelPolicy(ctx context.Context, action domain.Action, p domain.Progress, authority Authority, generation context.Context) (Result, error) {
	result := Result{Progress: p}
	value, ok := action.ApparelPolicy()
	...
	if v.Unresolved {
		...
		evidence, err := e.apparelPolicy.ObserveApparelPolicy(ctx, Placement{action, v.Attempt, v.Snapshot, v.Tick}, current)
		if err != nil {
			return result, err
		}
		if ctx.Err() != nil {
		...
		case domain.EffectCompleted:
			...
			if !evidence.Complete || evidence.Pawn != value.Pawn() || !known || !allowed {
				return result, ErrEvidence
			}
		case domain.EffectUnsuccessful:
			allowed, known := evidence.Matches.Value()
			if !evidence.Complete || evidence.Pawn != value.Pawn() || !known || allowed || o.UnsuccessfulReason != domain.OutcomeNotAchieved {
				return result, ErrEvidence
			}
	...
	var inspection ApparelPolicyInspection
	for range 2 {
		if err := e.guard(ctx, expected, generation); err != nil {
			return result, err
		}
		var err error
		inspection, err = e.apparelPolicy.InspectApparelPolicy(ctx, Target{action, expected})
		if err != nil {
			return result, err
		}
		if err = e.guard(ctx, expected, generation); err != nil {
		...
		decision := policy.EvaluateApparelPolicy(policy.ApparelPolicyRequest{Action: action, Progress: result.Progress, Current: expected, MinimumTick: v.Tick, Facts: inspection.Facts})
		result.Refused = decision.Refused
		if accepted, known := inspection.Facts.Accepted.Value(); known && !accepted {
		...
		admission := store.ApparelPolicyAdmission{Snapshot: expected, Tick: inspection.Facts.Tick, Value: value.Encoded()}
		next, err := e.apparelPolicyJournal.PrepareApparelPolicy(ctx, v.Plan, v.Action, admission)
		if err != nil {
			return result, err
		}
		result.Progress = next
	...
	next, err := e.journal.Dispatch(ctx, v.Plan, v.Action, expected, inspection.Facts.Tick)
	if err != nil {
		return result, err
	}
	result.Progress = next
	attempt := Placement{action, next.View().Attempt, expected, inspection.Facts.Tick}
	if err = e.guard(ctx, expected, generation); err != nil {
	...
	receipt, err := e.apparelPolicy.SetApparelPolicy(ctx, ApparelPolicyDispatch{Attempt: attempt})
	kind := receipt.Kind
	if err != nil {
