...
func (m *movementFake) InspectMovement(_ context.Context, target Target, claim domain.DraftClaim) (MovementInspection, error) {
	...
	v := MovementInspection{StartedAt: now, ObservedAt: now, Facts: policy.MovementFacts{Snapshot: target.Snapshot, PawnTick: tick, PreviewTick: tick, Emergency: emergency, NativeCanTry: domain.Known(true), Pawn: policy.MovementPawnFacts{Pawn: "pawn", SnapshotToken: "pawn-cas", Dead: domain.Known(false), Downed: domain.Known(false), Bleeding: domain.Known(false), NeedsTend: domain.Known(false), FreeColonist: domain.Known(m.freeColonist), Drafted: domain.Known(true), Owner: domain.Known(policy.MovementDraftOwner{Claim: claim.Claim, Session: claim.Session})}}}
	if m.inspect != nil {
		m.inspect(&v)
	}
	return v, nil
