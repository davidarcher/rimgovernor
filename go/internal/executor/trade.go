...
//
//  1. The plan's own domain.ActionDependency on a sibling trade action,
//     declared by whoever built a multi-action trade plan. Unchanged, and
//     still the only binding such a plan needs. A dependency on any other
//     sibling (the escort goto the routine's accept waits for) is ordering
//     only and names no session.
...
func (e *Executor) resolveTradeDependency(ctx context.Context, plan domain.PlanID, action domain.ActionID, kind domain.TradeOperationKind) (TradeDependency, error) {
	...
	found := false
	tradeActions := map[domain.ActionID]bool{}
	for _, a := range state.Spec.Actions() {
		tradeActions[a.ID()] = a.Kind() == domain.TradeAction
	}
	for _, d := range state.Spec.Dependencies() {
		if d.Action == action && tradeActions[d.Requires] {
