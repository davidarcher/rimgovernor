...
func (e *Executor) runRescue(ctx context.Context, action domain.Action, p domain.Progress, authority Authority, generation context.Context) (Result, error) {
	...
	v := p.View()
	if v.Unresolved {
		return e.reconcileRescue(ctx, result, generation)
	}
	switch v.Stage {
	...
	minimum := v.Tick
	var inspection RescueInspection
	for range 2 {
		if err := e.guard(ctx, expected, generation); err != nil {
			return result, err
		}
		var err error
		inspection, err = e.rescue.InspectRescue(ctx, Target{action, expected})
		if err != nil {
			return result, err
		}
		if err = e.guard(ctx, expected, generation); err != nil {
		...
		next, err := e.rescueJournal.PrepareRescue(ctx, v.Plan, v.Action, admission)
		if err != nil {
			return result, err
		}
	...
	admission := store.RescueAdmission{Snapshot: expected, Tick: inspection.Facts.PreviewTick, Rescuer: rescue.Rescuer(), Patient: rescue.Patient(), RescuerSnapshotToken: inspection.Facts.Rescuer.SnapshotToken, PatientSnapshotToken: inspection.Facts.Patient.SnapshotToken}
	if err = e.guard(ctx, expected, generation); err != nil {
	...
	receipt, err := e.rescue.RescuePatient(ctx, RescueDispatch{attempt, admission})
	kind := receipt.Kind
	if err != nil {
...
func (e *Executor) reconcileRescue(ctx context.Context, result Result, generation context.Context) (Result, error) {
	p := result.Progress
	v := p.View()
	...
	var admission store.RescueAdmission
	found := false
	...
	evidence, err := e.rescue.ObserveRescue(ctx, RescueDispatch{Placement{p.Action(), v.Attempt, v.Snapshot, v.Tick}, admission}, current)
	if err != nil {
		return result, err
	}
	if ctx.Err() != nil {
	...
	rescue, _ := p.Action().Rescue()
	o := evidence.Observation
	if o.Action != v.Action || o.Attempt != v.Attempt || o.Snapshot != current || evidence.Rescuer != rescue.Rescuer() || evidence.Patient != rescue.Patient() || !e.fresh(evidence.StartedAt, evidence.ObservedAt) {
		return result, ErrEvidence
	}
	switch o.Effect {
