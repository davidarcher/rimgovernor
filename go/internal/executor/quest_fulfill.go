...
func (e *Executor) runQuestFulfill(ctx context.Context, action domain.Action, p domain.Progress, authority Authority, generation context.Context) (Result, error) {
	result := Result{Progress: p}
	fulfill, ok := action.QuestFulfill()
	if !ok || p.Action() != action {
		return result, ErrEvidence
	}
	v := p.View()
	if v.Unresolved {
		return e.reconcileQuestFulfill(ctx, result, generation)
	}
	switch v.Stage {
	...
	minimum := v.Tick
	var inspection QuestFulfillInspection
	for range 2 {
		if err := e.guard(ctx, expected, generation); err != nil {
			return result, err
		}
		var err error
		inspection, err = e.questFulfill.InspectQuestFulfill(ctx, Target{action, expected})
		if err != nil {
			return result, err
		}
		if err = e.guard(ctx, expected, generation); err != nil {
		...
		minimum = max(minimum, facts.QuestTick, facts.CaravanTick)
		...
		next, err := e.questFulfillJournal.PrepareQuestFulfill(ctx, v.Plan, v.Action, admission)
		if err != nil {
			return result, err
		}
	...
	admission := store.QuestFulfillAdmission{Snapshot: expected, Tick: inspection.Facts.PreviewTick, Quest: fulfill.Quest(), Caravan: fulfill.Caravan(), CrewIDs: fulfill.CrewIDs(), QuestSnapshotToken: inspection.Facts.Fulfill.QuestToken, CaravanSnapshotToken: inspection.Facts.Fulfill.CaravanToken}
	if err = e.guard(ctx, expected, generation); err != nil {
	...
	receipt, err := e.questFulfill.WriteQuestFulfill(ctx, QuestFulfillDispatch{attempt, admission})
	kind := receipt.Kind
	if err != nil {
...
func (e *Executor) reconcileQuestFulfill(ctx context.Context, result Result, generation context.Context) (Result, error) {
	p := result.Progress
	v := p.View()
	...
	var admission store.QuestFulfillAdmission
	found := false
	...
	evidence, err := e.questFulfill.ObserveQuestFulfill(ctx, QuestFulfillDispatch{Placement{p.Action(), v.Attempt, v.Snapshot, v.Tick}, admission}, current)
	if err != nil {
		return result, err
	}
	if ctx.Err() != nil {
	...
	fulfill, _ := p.Action().QuestFulfill()
	o := evidence.Observation
	if o.Action != v.Action || o.Attempt != v.Attempt || o.Snapshot != current || evidence.Quest != fulfill.Quest() || !e.fresh(evidence.StartedAt, evidence.ObservedAt) {
		return result, ErrEvidence
	}
	switch o.Effect {
