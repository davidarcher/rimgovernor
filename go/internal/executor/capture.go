...
func (e *Executor) runCapture(ctx context.Context, action domain.Action, p domain.Progress, authority Authority, generation context.Context) (Result, error) {
	...
	v := p.View()
	if v.Unresolved {
		return e.reconcileCapture(ctx, result, generation)
	}
	switch v.Stage {
	...
	minimum := v.Tick
	var inspection CaptureInspection
	for range 2 {
		if err := e.guard(ctx, expected, generation); err != nil {
			return result, err
		}
		var err error
		inspection, err = e.capture.InspectCapture(ctx, Target{action, expected})
		if err != nil {
			return result, err
		}
		if err = e.guard(ctx, expected, generation); err != nil {
		...
		next, err := e.captureJournal.PrepareCapture(ctx, v.Plan, v.Action, admission)
		if err != nil {
			return result, err
		}
	...
	admission := store.CaptureAdmission{Snapshot: expected, Tick: inspection.Facts.PreviewTick, Capturer: capture.Capturer(), Patient: capture.Patient(), CapturerSnapshotToken: inspection.Facts.Capturer.SnapshotToken, PatientSnapshotToken: inspection.Facts.Patient.SnapshotToken}
	if err = e.guard(ctx, expected, generation); err != nil {
...
func (e *Executor) reconcileCapture(ctx context.Context, result Result, generation context.Context) (Result, error) {
	p := result.Progress
	v := p.View()
	...
	var admission store.CaptureAdmission
	found := false
	for _, record := range state.CaptureAdmissions {
	...
	evidence, err := e.capture.ObserveCapture(ctx, CaptureDispatch{Placement{p.Action(), v.Attempt, v.Snapshot, v.Tick}, admission}, current)
	if err != nil {
		return result, err
	}
	if ctx.Err() != nil {
	...
	if o.Action != v.Action || o.Attempt != v.Attempt || o.Snapshot != current || evidence.Capturer != capture.Capturer() || evidence.Patient != capture.Patient() || !e.fresh(evidence.StartedAt, evidence.ObservedAt) {
		return result, ErrEvidence
	}
	switch o.Effect {
