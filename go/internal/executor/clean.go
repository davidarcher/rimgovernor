...
func (e *Executor) runClean(ctx context.Context, action domain.Action, p domain.Progress, authority Authority, generation context.Context) (Result, error) {
	...
	v := p.View()
	if v.Unresolved {
		return e.reconcileClean(ctx, result, generation)
	}
	switch v.Stage {
	...
	for range 2 {
		...
		var err error
		inspection, err = e.clean.InspectClean(ctx, Target{action, expected})
		if err != nil {
			return result, err
		}
		if err = e.guard(ctx, expected, generation); err != nil {
		...
		next, err := e.cleanJournal.PrepareClean(ctx, v.Plan, v.Action, admission)
		if err != nil {
			return result, err
		}
	...
	admission := store.CleanAdmission{Snapshot: expected, Tick: inspection.Facts.PreviewTick, Pawn: clean.Pawn(), Filth: clean.Filth(), Cell: clean.Cell(), PawnSnapshotToken: inspection.Facts.Pawn.SnapshotToken, FilthSnapshotToken: inspection.Facts.Filth.SnapshotToken}
	if err = e.guard(ctx, expected, generation); err != nil {
	...
	receipt, err := e.clean.CleanFilth(ctx, CleanDispatch{attempt, admission})
	kind := receipt.Kind
	if err != nil {
...
func (e *Executor) reconcileClean(ctx context.Context, result Result, generation context.Context) (Result, error) {
	p := result.Progress
	v := p.View()
	...
	var admission store.CleanAdmission
	found := false
	...
	evidence, err := e.clean.ObserveClean(ctx, CleanDispatch{Placement{p.Action(), v.Attempt, v.Snapshot, v.Tick}, admission}, current)
	if err != nil {
		return result, err
	}
	if ctx.Err() != nil {
	...
	clean, _ := p.Action().Clean()
	o := evidence.Observation
	if o.Action != v.Action || o.Attempt != v.Attempt || o.Snapshot != current || evidence.Pawn != clean.Pawn() || evidence.Filth != clean.Filth() || !e.fresh(evidence.StartedAt, evidence.ObservedAt) {
		return result, ErrEvidence
	}
	switch o.Effect {
