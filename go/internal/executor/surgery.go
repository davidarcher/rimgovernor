...
func (e *Executor) runSurgery(ctx context.Context, action domain.Action, p domain.Progress, authority Authority, generation context.Context) (Result, error) {
	result := Result{Progress: p}
	surgery, ok := action.Surgery()
	if !ok || p.Action() != action {
		return result, ErrEvidence
	}
	v := p.View()
	if v.Unresolved {
		return e.reconcileSurgery(ctx, result, generation)
	}
	switch v.Stage {
	...
	minimum := v.Tick
	var inspection SurgeryInspection
	for range 2 {
		if err := e.guard(ctx, expected, generation); err != nil {
			return result, err
		}
		var err error
		inspection, err = e.surgery.InspectSurgery(ctx, Target{action, expected})
		if err != nil {
			return result, err
		}
		if err = e.guard(ctx, expected, generation); err != nil {
		...
		decision := policy.EvaluateSurgery(policy.SurgeryRequest{Action: action, Progress: result.Progress, Current: expected, MinimumTick: minimum, Facts: facts})
		result.Refused = decision.Refused
		if !decision.Admitted {
			return result, ErrHeld
		}
		admission := store.SurgeryAdmission{Snapshot: expected, Tick: facts.PreviewTick, Patient: surgery.Patient(), Recipe: surgery.Recipe(), Part: surgery.Part(), PatientSnapshotToken: facts.Patient.SnapshotToken, HealthToken: facts.HealthToken, Care: facts.Care}
		next, err := e.surgeryJournal.PrepareSurgery(ctx, v.Plan, v.Action, admission)
		if err != nil {
			return result, err
		}
	...
	admission := store.SurgeryAdmission{Snapshot: expected, Tick: inspection.Facts.PreviewTick, Patient: surgery.Patient(), Recipe: surgery.Recipe(), Part: surgery.Part(), PatientSnapshotToken: inspection.Facts.Patient.SnapshotToken, HealthToken: inspection.Facts.HealthToken, Care: inspection.Facts.Care}
	if err = e.guard(ctx, expected, generation); err != nil {
	...
	receipt, err := e.surgery.QueueSurgery(ctx, SurgeryDispatch{attempt, admission})
	kind := receipt.Kind
	if err != nil {
...
func (e *Executor) reconcileSurgery(ctx context.Context, result Result, generation context.Context) (Result, error) {
	p := result.Progress
	v := p.View()
	...
	var admission store.SurgeryAdmission
	found := false
	...
	evidence, err := e.surgery.ObserveSurgery(ctx, SurgeryDispatch{Placement{p.Action(), v.Attempt, v.Snapshot, v.Tick}, admission}, current)
	if err != nil {
		return result, err
	}
	if ctx.Err() != nil {
	...
	surgery, _ := p.Action().Surgery()
	o := evidence.Observation
	if o.Action != v.Action || o.Attempt != v.Attempt || o.Snapshot != current || evidence.Patient != surgery.Patient() || !e.fresh(evidence.StartedAt, evidence.ObservedAt) {
		return result, ErrEvidence
	}
	switch o.Effect {
