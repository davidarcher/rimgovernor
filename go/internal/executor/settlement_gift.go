...
func (e *Executor) runSettlementGift(ctx context.Context, action domain.Action, p domain.Progress, authority Authority, generation context.Context) (Result, error) {
	result := Result{Progress: p}
	gift, ok := action.SettlementGift()
	if !ok || p.Action() != action {
		return result, ErrEvidence
	}
	v := p.View()
	if v.Unresolved {
		return e.reconcileSettlementGift(ctx, result, generation)
	}
	switch v.Stage {
	...
	minimum := v.Tick
	var inspection SettlementGiftInspection
	for range 2 {
		if err := e.guard(ctx, expected, generation); err != nil {
			return result, err
		}
		var err error
		inspection, err = e.settlementGift.InspectSettlementGift(ctx, Target{action, expected})
		if err != nil {
			return result, err
		}
		if err = e.guard(ctx, expected, generation); err != nil {
		...
		minimum = max(minimum, facts.CaravanTick, facts.WorldTick)
		decision := policy.EvaluateSettlementGift(policy.SettlementGiftRequest{Action: action, Progress: result.Progress, Current: expected, MinimumTick: minimum, Facts: facts})
		...
		admission := store.SettlementGiftAdmission{
			Snapshot: expected, Tick: facts.PreviewTick, Caravan: gift.Caravan(), Settlement: gift.Settlement(), Faction: gift.Faction(),
			CrewIDs: gift.CrewIDs(), Silver: gift.Silver(), CaravanSnapshotToken: facts.Gift.CaravanToken, FactionSnapshotToken: facts.Gift.FactionToken,
		}
		next, err := e.settlementGiftJournal.PrepareSettlementGift(ctx, v.Plan, v.Action, admission)
		if err != nil {
			return result, err
		}
	...
	admission := store.SettlementGiftAdmission{
		Snapshot: expected, Tick: inspection.Facts.PreviewTick, Caravan: gift.Caravan(), Settlement: gift.Settlement(), Faction: gift.Faction(),
		CrewIDs: gift.CrewIDs(), Silver: gift.Silver(), CaravanSnapshotToken: inspection.Facts.Gift.CaravanToken, FactionSnapshotToken: inspection.Facts.Gift.FactionToken,
	}
	if err = e.guard(ctx, expected, generation); err != nil {
	...
	receipt, err := e.settlementGift.WriteSettlementGift(ctx, SettlementGiftDispatch{attempt, admission})
	kind := receipt.Kind
	if err != nil {
...
func (e *Executor) reconcileSettlementGift(ctx context.Context, result Result, generation context.Context) (Result, error) {
	p := result.Progress
	v := p.View()
	...
	var admission store.SettlementGiftAdmission
	found := false
	...
	evidence, err := e.settlementGift.ObserveSettlementGift(ctx, SettlementGiftDispatch{Placement{p.Action(), v.Attempt, v.Snapshot, v.Tick}, admission}, current)
	if err != nil {
		return result, err
	}
	if ctx.Err() != nil {
	...
	gift, _ := p.Action().SettlementGift()
	o := evidence.Observation
	if o.Action != v.Action || o.Attempt != v.Attempt || o.Snapshot != current || evidence.Caravan != gift.Caravan() || !e.fresh(evidence.StartedAt, evidence.ObservedAt) {
		return result, ErrEvidence
	}
	switch o.Effect {
