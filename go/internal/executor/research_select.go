...
func (e *Executor) runResearchSelect(ctx context.Context, action domain.Action, p domain.Progress, authority Authority, generation context.Context) (Result, error) {
	result := Result{Progress: p}
	value, ok := action.ResearchSelect()
	...
	if v.Unresolved {
		...
		evidence, err := e.researchSelect.ObserveResearchSelect(ctx, Placement{action, v.Attempt, v.Snapshot, v.Tick}, current)
		if err != nil {
			return result, err
		}
		if ctx.Err() != nil {
		...
		case domain.EffectCompleted:
			...
			if !evidence.Complete || evidence.Project != value.Project() || !known || !allowed {
				return result, ErrEvidence
			}
		case domain.EffectUnsuccessful:
			allowed, known := evidence.Matches.Value()
			if !evidence.Complete || evidence.Project != value.Project() || !known || allowed || o.UnsuccessfulReason != domain.OutcomeNotAchieved {
				return result, ErrEvidence
			}
	...
	var inspection ResearchSelectInspection
	for range 2 {
		if err := e.guard(ctx, expected, generation); err != nil {
			return result, err
		}
		var err error
		inspection, err = e.researchSelect.InspectResearchSelect(ctx, Target{action, expected})
		if err != nil {
			return result, err
		}
		if err = e.guard(ctx, expected, generation); err != nil {
		...
		next, err := e.researchSelectJournal.PrepareResearchSelect(ctx, v.Plan, v.Action, admission)
		if err != nil {
			return result, err
		}
		result.Progress = next
	...
	receipt, err := e.researchSelect.SelectResearch(ctx, ResearchSelectDispatch{attempt, value.Project(), inspection.Token})
	kind := receipt.Kind
	if err != nil {
