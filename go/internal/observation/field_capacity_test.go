...
func TestUnsownFieldCapacityDoesNotBecomeGrowingFood(t *testing.T) {
	...
	growing := policy.FieldCoverage(domain.Known(int64(3)), colonyFieldCrops(v, definitions), 7)
	capacity := policy.FieldCoverage(domain.Known(int64(3)), colonyFieldCrops(v, definitions, true), 7)
	if growing != domain.Known(0.0) || capacity != domain.Known(1.0) {
		t.Fatal(growing, capacity)
	}
	v.Farms[0].UsableCells = nil
	if _, known := policy.FieldCoverage(domain.Known(int64(3)), colonyFieldCrops(v, definitions, true), 7).Value(); known {
