...
func TestColonyCalendarReachesRoutineFacts(t *testing.T) {
	...
	if !known || calendar != (policy.Calendar{Season: "Fall", DayOfYear: 35, GrowingDays: 40, GrowingDaysRemaining: 10, GrowingDaysUntil: 0, NonGrowingDays: 22, Sowing: true}) {
		t.Fatal(p.Facts.Calendar)
	}
	if climate, known := p.CropClimate.DaysRemaining.Value(); !known || climate != 10 {
		t.Fatal(p.CropClimate)
	}
	// The wait until growth resumes and the non-growing stretch are part of
	// the calendar: without either the pre-calendar climate keeps the crop
	// plan but the routine gets no fact.
