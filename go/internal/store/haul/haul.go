// Package haul holds the haul action family's admission record, validation
...
func ValidateAdmission(a domain.Action, p domain.Progress, admission Admission) error {
	...
	h, ok := a.Haul()
	if !ok || h.Pawn() != admission.Pawn || h.Thing() != admission.Thing || h.Definition() != admission.Definition || h.Cell() != admission.Cell || idShaped(admission.PawnSnapshotToken) != nil || idShaped(admission.ThingSnapshotToken) != nil {
...
func LoadAdmission(ctx context.Context, tx *sql.Tx, a domain.Action, p domain.Progress) (Admission, bool, error) {
	...
	if len(data) > 32768 {
		return Admission{}, false, errors.New("haul admission exceeds bound")
	}
...
func Insert(ctx context.Context, tx *sql.Tx, action domain.ActionID, admission Admission) error {
	...
	_, err = tx.ExecContext(ctx, "INSERT INTO haul_admissions(action_id,payload) VALUES(?,?) ON CONFLICT(action_id) DO UPDATE SET payload=excluded.payload", action, data)
	return err
}
