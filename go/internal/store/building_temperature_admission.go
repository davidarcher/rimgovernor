...
type BuildingTemperatureAdmission = buildingtemperature.Admission
type ActionBuildingTemperatureAdmission = buildingtemperature.ActionAdmission
...
func (s *Store) PrepareBuildingTemperature(ctx context.Context, plan domain.PlanID, action domain.ActionID, admission BuildingTemperatureAdmission) (domain.Progress, error) {
	...
	if err = buildingtemperature.ValidateAdmission(a, p, admission); err != nil {
		return domain.Progress{}, err
	}
	v := p.View()
	...
	for _, old := range state.BuildingTemperatureAdmissions {
	...
	if err = buildingtemperature.Insert(ctx, tx, action, admission); err != nil {
		return domain.Progress{}, err
	}
