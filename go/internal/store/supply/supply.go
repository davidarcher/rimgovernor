// Package supply holds the supply-allow action family's admission record,
...
func ValidateAdmission(a domain.Action, p domain.Progress, admission Admission) error {
	...
	if !ok || supply.Thing() != admission.Thing || idShaped(admission.SnapshotToken) != nil || admission.Snapshot.Native == 0 || admission.Snapshot.Direction == 0 {
...
func LoadAdmission(ctx context.Context, tx *sql.Tx, a domain.Action, p domain.Progress) (Admission, bool, error) {
	...
	if len(data) > 32768 {
		return Admission{}, false, errors.New("supply admission exceeds bound")
	}
...
func Insert(ctx context.Context, tx *sql.Tx, action domain.ActionID, admission Admission) error {
	...
	_, err = tx.ExecContext(ctx, "INSERT INTO supply_admissions(action_id,payload) VALUES(?,?) ON CONFLICT(action_id) DO UPDATE SET payload=excluded.payload", action, data)
	return err
}
