...
// receipt may prepare again, while unknown, completed and cancelled zone
...
func (s *Store) PrepareZone(ctx context.Context, plan domain.PlanID, action domain.ActionID, admission ZoneAdmission) (domain.Progress, error) {
	tx, err := s.begin(ctx)
	if err != nil {
	...
	default:
		return domain.Progress{}, errors.New("admission replacement requires pending or prepared zone")
	}
	// Retain the newest validated observation even when preparation predates it.
	for _, old := range state.ZoneAdmissions {
