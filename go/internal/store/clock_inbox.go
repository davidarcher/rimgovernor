...
func checkClockInboxSchema(ctx context.Context, tx *sql.Tx) error {
...
func (s *Store) BindClockInbox(ctx context.Context, profile string) (ClockInboxState, error) {
	...
	if errors.Is(err, sql.ErrNoRows) {
		...
		return ClockInboxState{}, ErrConflict
	}
	if err != nil {
		return ClockInboxState{}, err
	}
...
func (s *Store) ReadClockInbox(ctx context.Context, profile string) (ClockInboxState, error) {
	inbox, err := s.LoadClockInbox(ctx, profile, clockInboxCapacity)
...
func (s *Store) LoadClockInbox(ctx context.Context, profile string, limit int) (ClockInbox, error) {
	if limit < 1 || limit > clockInboxCapacity {
		return ClockInbox{}, errors.New("invalid clock inbox limit")
	}
	path, err := canonicalClockProfile(profile)
	if err != nil {
...
func (s *Store) AppendClockEvents(ctx context.Context, profile string, request *k.EventsRequest, page *k.EventsPage) (ClockInboxState, bool, error) {
	...
	if request.GetAfterCursor() != inbox.State.Cursor {
		return ClockInboxState{}, false, ErrConflict
	}
	...
	for _, event := range page.Events {
		...
		if e != nil {
			return ClockInboxState{}, false, e
		}
...
func loadClockInbox(ctx context.Context, tx *sql.Tx, profile string, limit int) (ClockInbox, int64, error) {
	var inbox ClockInbox
	...
	for rows.Next() {
		...
		inbox.Pages = append(inbox.Pages, ClockEventPage{Request: request, Page: page})
