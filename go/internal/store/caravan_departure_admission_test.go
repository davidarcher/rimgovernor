...
func caravanDepartureStoreFixture(t *testing.T) (*Store, string, CaravanDepartureAdmission) {
	...
	v := CaravanDepartureAdmission{Snapshot: snapshot, Tick: 12, Crew: []CaravanCrewAdmission{{Pawn: "alpha", SnapshotToken: "cas-alpha"}, {Pawn: "beta", SnapshotToken: "cas-beta"}}, CatalogToken: "catalog-cas", Cargo: []CaravanCargoAdmission{{GroupID: "g-meals", Definition: "MealSimple", Count: 10}, {GroupID: "g-pemmican", Definition: "Pemmican", Count: 24}}}
	return s, path, v
}
...
// TestCaravanDepartureAdmissionCargoCoversAction: the composed pack must
// carry every action cargo line in full (#464).
func TestCaravanDepartureAdmissionCargoCoversAction(t *testing.T) {
	ctx := context.Background()
	for name, change := range map[string]func(*CaravanDepartureAdmission){
		"empty":     func(v *CaravanDepartureAdmission) { v.Cargo = nil },
		"short":     func(v *CaravanDepartureAdmission) { v.Cargo[0].Count = 9 },
		"missing":   func(v *CaravanDepartureAdmission) { v.Cargo = v.Cargo[1:] },
		"duplicate": func(v *CaravanDepartureAdmission) { v.Cargo[1].GroupID = "g-meals" },
		"zero":      func(v *CaravanDepartureAdmission) { v.Cargo[1].Count = 0 },
	} {
