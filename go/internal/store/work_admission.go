...
// receipt may prepare again, while unknown, completed and cancelled work
...
func (s *Store) PrepareWork(ctx context.Context, plan domain.PlanID, action domain.ActionID, admission WorkAdmission) (domain.Progress, error) {
	tx, err := s.begin(ctx)
	if err != nil {
	...
	for _, old := range state.WorkAdmissions {
