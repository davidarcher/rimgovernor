...
type SupplyAdmission = supply.Admission
type ActionSupplyAdmission = supply.ActionAdmission
...
func (s *Store) PrepareSupply(ctx context.Context, plan domain.PlanID, action domain.ActionID, admission SupplyAdmission) (domain.Progress, error) {
	...
	if err = supply.ValidateAdmission(a, p, admission); err != nil {
		return domain.Progress{}, err
	}
	v := p.View()
	...
	if err = supply.Insert(ctx, tx, action, admission); err != nil {
		return domain.Progress{}, err
	}
	if prepare {
