...
type ZoneEditAdmission = zoneedit.Admission
type ActionZoneEditAdmission = zoneedit.ActionAdmission

// PrepareZoneEdit atomically records the exact zone CAS evidence and
// prepares pending work, mirroring PrepareBuildingTemperature's one-shot
// single-entity admission shape. This record is evidence, not a lease: the
// executor must inspect and prepare again before dispatch after restart,
// even when Prepared was persisted.
func (s *Store) PrepareZoneEdit(ctx context.Context, plan domain.PlanID, action domain.ActionID, admission ZoneEditAdmission) (domain.Progress, error) {
	tx, err := s.begin(ctx)
	if err != nil {
	...
	if err = zoneedit.ValidateAdmission(a, p, admission); err != nil {
		return domain.Progress{}, err
	}
	v := p.View()
	...
	for _, old := range state.ZoneEditAdmissions {
	...
	if err = zoneedit.Insert(ctx, tx, action, admission); err != nil {
		return domain.Progress{}, err
	}
