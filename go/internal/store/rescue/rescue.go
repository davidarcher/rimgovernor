// Package rescue holds the rescue action family's admission record,
...
func ValidateAdmission(a domain.Action, p domain.Progress, admission Admission) error {
	...
	r, ok := a.Rescue()
	if !ok || r.Rescuer() != admission.Rescuer || r.Patient() != admission.Patient || idShaped(admission.RescuerSnapshotToken) != nil || idShaped(admission.PatientSnapshotToken) != nil {
...
func LoadAdmission(ctx context.Context, tx *sql.Tx, a domain.Action, p domain.Progress) (Admission, bool, error) {
	...
	if len(data) > 32768 {
		return Admission{}, false, errors.New("rescue admission exceeds bound")
	}
...
func Insert(ctx context.Context, tx *sql.Tx, action domain.ActionID, admission Admission) error {
	...
	_, err = tx.ExecContext(ctx, "INSERT INTO rescue_admissions(action_id,payload) VALUES(?,?) ON CONFLICT(action_id) DO UPDATE SET payload=excluded.payload", action, data)
	return err
}
