...
// CaravanCargoAdmission is one FormCaravan cargo line of the pack the
// boundary composed at inspect time: the action's own cargo plus the crew's
// journey food (policy.PlanCaravanCargo, #464). Dispatch loads exactly these
// lines against CatalogToken.
type CaravanCargoAdmission struct {
	GroupID    string
	Definition string
	Count      int64
}

// CaravanDepartureAdmission records the exact crew CAS evidence, the catalog
// token and the composed cargo a CaravanDeparture action was prepared against.
type CaravanDepartureAdmission struct {
	Snapshot     domain.GenerationSnapshot
	Tick         domain.Tick
	Crew         []CaravanCrewAdmission
	CatalogToken string
	Cargo        []CaravanCargoAdmission
}
...
func validateCaravanDepartureAdmission(a domain.Action, p domain.Progress, admission CaravanDepartureAdmission) error {
	...
	if len(admission.Cargo) == 0 || len(admission.Cargo) > 256 {
		return errors.New("invalid caravan departure admission cargo")
	}
	packed := make(map[string]uint64, len(admission.Cargo))
	groups := make(map[string]bool, len(admission.Cargo))
	for _, line := range admission.Cargo {
		if submissionID(line.GroupID) != nil || submissionID(line.Definition) != nil || line.Count <= 0 || groups[line.GroupID] {
			return errors.New("invalid caravan departure admission cargo line")
		}
		groups[line.GroupID] = true
		packed[line.Definition] += uint64(line.Count)
	}
	for _, item := range departure.Cargo() {
		if packed[item.Definition] < item.Count {
			return errors.New("caravan departure admission cargo omits action cargo")
		}
	}
	return nil
}
