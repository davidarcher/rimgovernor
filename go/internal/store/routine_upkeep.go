...
func routineUpkeepIssued(ctx context.Context, tx *sql.Tx, current domain.GenerationSnapshot) (map[policy.GoalID]bool, error) {
	...
	for _, plan := range plans {
		...
		for _, need := range []policy.GoalID{policy.MaintainFireSafety, policy.SecureSupplies, policy.MaintainEssentialRepairs, policy.MaintainCleanFacilities, policy.MaintainStorage, policy.MaintainMedicalReserves, policy.MaintainAnimalContainment, policy.MaintainAnimalFeed, policy.MaintainSleeping, policy.MaintainHomeCoverage, policy.MaintainStoneShell} {
