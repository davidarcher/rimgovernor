// Package tend holds the tend action family's admission record, validation
// and load logic, split out of internal/store for independent build/test
// caching.
package tend
...
func ValidateAdmission(a domain.Action, p domain.Progress, admission Admission) error {
	...
	t, ok := a.Tend()
	if !ok || t.Doctor() != admission.Doctor || t.Patient() != admission.Patient || idShaped(admission.DoctorSnapshotToken) != nil || idShaped(admission.PatientSnapshotToken) != nil {
...
func LoadAdmission(ctx context.Context, tx *sql.Tx, a domain.Action, p domain.Progress) (Admission, bool, error) {
	...
	if len(data) > 32768 {
		return Admission{}, false, errors.New("tend admission exceeds bound")
	}
...
func Insert(ctx context.Context, tx *sql.Tx, action domain.ActionID, admission Admission) error {
	...
	_, err = tx.ExecContext(ctx, "INSERT INTO tend_admissions(action_id,payload) VALUES(?,?) ON CONFLICT(action_id) DO UPDATE SET payload=excluded.payload", action, data)
	return err
}
