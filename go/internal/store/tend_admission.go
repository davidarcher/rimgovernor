...
type TendAdmission struct {
	Snapshot                                  domain.GenerationSnapshot
	Tick                                      domain.Tick
	Doctor, Patient                           domain.PawnID
	DoctorSnapshotToken, PatientSnapshotToken string
}
type ActionTendAdmission struct {
	Action    domain.ActionID
	Admission TendAdmission
}

func validateTendAdmission(a domain.Action, p domain.Progress, admission TendAdmission) error {
	...
	tend, ok := a.Tend()
	if !ok || tend.Doctor() != admission.Doctor || tend.Patient() != admission.Patient || submissionID(admission.DoctorSnapshotToken) != nil || submissionID(admission.PatientSnapshotToken) != nil {
		return errors.New("invalid tend admission")
	}
	return nil
}

// PrepareTend atomically records the exact doctor/patient CAS evidence and
...
func (s *Store) PrepareTend(ctx context.Context, plan domain.PlanID, action domain.ActionID, admission TendAdmission) (domain.Progress, error) {
	tx, err := s.begin(ctx)
	if err != nil {
	...
	if err = validateTendAdmission(a, p, admission); err != nil {
		return domain.Progress{}, err
	}
	v := p.View()
	...
	for _, old := range state.TendAdmissions {
	...
	if _, err = tx.ExecContext(ctx, "INSERT INTO tend_admissions(action_id,payload) VALUES(?,?) ON CONFLICT(action_id) DO UPDATE SET payload=excluded.payload", action, data); err != nil {
		return domain.Progress{}, err
	}
...
func loadTendAdmission(ctx context.Context, tx *sql.Tx, a domain.Action, p domain.Progress) (TendAdmission, bool, error) {
	var data []byte
	if err := tx.QueryRowContext(ctx, "SELECT payload FROM tend_admissions WHERE action_id=?", a.ID()).Scan(&data); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return TendAdmission{}, false, nil
		}
		return TendAdmission{}, false, err
	}
	var admission TendAdmission
	if len(data) > 32768 {
		return TendAdmission{}, false, errors.New("tend admission exceeds bound")
	}
	...
	if err := decoder.Decode(new(json.RawMessage)); err != io.EOF {
		return TendAdmission{}, false, errors.New("trailing admission data")
	}
	...
	if !bytes.Equal(data, canonical) {
		return TendAdmission{}, false, errors.New("noncanonical admission record")
	}
	if err = validateTendAdmission(a, p, admission); err != nil {
		return TendAdmission{}, false, fmt.Errorf("invalid action %q admission: %w", a.ID(), err)
	}
	v := p.View()
	if (v.Stage == domain.Prepared || v.Attempt > 0) && !admission.Snapshot.Matches(v.Snapshot) {
		return TendAdmission{}, false, errors.New("admission and progress authority disagree")
	}
	if (v.Unresolved || v.Stage == domain.Completed || v.Stage == domain.Unsuccessful) && admission.Tick > v.Tick {
		return TendAdmission{}, false, errors.New("admission is newer than dispatched progress")
	}
