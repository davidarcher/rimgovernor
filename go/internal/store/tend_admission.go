...
type TendAdmission = tend.Admission
type ActionTendAdmission = tend.ActionAdmission
...
func (s *Store) PrepareTend(ctx context.Context, plan domain.PlanID, action domain.ActionID, admission TendAdmission) (domain.Progress, error) {
	...
	if err = tend.ValidateAdmission(a, p, admission); err != nil {
		return domain.Progress{}, err
	}
	v := p.View()
	...
	if err = tend.Insert(ctx, tx, action, admission); err != nil {
		return domain.Progress{}, err
	}
