// Package zoneedit holds the zone-edit action family's admission record,
// validation and load logic, split out of internal/store for independent
// build/test caching -- mirroring internal/store/buildingtemperature.
package zoneedit
...
func ValidateAdmission(a domain.Action, p domain.Progress, admission Admission) error {
	...
	edit, ok := a.ZoneEdit()
	if !ok || edit.ZoneID() != admission.ZoneID || idShaped(admission.SnapshotToken) != nil || admission.Snapshot.Native == 0 || admission.Snapshot.Direction == 0 {
		return errors.New("invalid zone edit admission")
	}
	return nil
}
...
func LoadAdmission(ctx context.Context, tx *sql.Tx, a domain.Action, p domain.Progress) (Admission, bool, error) {
	var data []byte
	if err := tx.QueryRowContext(ctx, "SELECT payload FROM zone_edit_admissions WHERE action_id=?", a.ID()).Scan(&data); err != nil {
	...
	if len(data) > 32768 {
		return Admission{}, false, errors.New("zone edit admission exceeds bound")
	}
...
func Insert(ctx context.Context, tx *sql.Tx, action domain.ActionID, admission Admission) error {
	...
	_, err = tx.ExecContext(ctx, "INSERT INTO zone_edit_admissions(action_id,payload) VALUES(?,?) ON CONFLICT(action_id) DO UPDATE SET payload=excluded.payload", action, data)
	return err
}
