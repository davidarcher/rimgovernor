// Package capture holds the capture action family's admission record,
// validation and load logic, split out of internal/store for independent
// build/test caching -- mirroring package rescue exactly, since Capture and
// Rescue share the identical PawnTargetOrder admission shape.
package capture
...
type Admission struct {
	...
	CapturerSnapshotToken, PatientSnapshotToken string
}
...
func ValidateAdmission(a domain.Action, p domain.Progress, admission Admission) error {
	...
	c, ok := a.Capture()
	if !ok || c.Capturer() != admission.Capturer || c.Patient() != admission.Patient || idShaped(admission.CapturerSnapshotToken) != nil || idShaped(admission.PatientSnapshotToken) != nil {
		return errors.New("invalid capture admission")
	}
	return nil
}
...
func LoadAdmission(ctx context.Context, tx *sql.Tx, a domain.Action, p domain.Progress) (Admission, bool, error) {
	var data []byte
	if err := tx.QueryRowContext(ctx, "SELECT payload FROM capture_admissions WHERE action_id=?", a.ID()).Scan(&data); err != nil {
	...
	if len(data) > 32768 {
		return Admission{}, false, errors.New("capture admission exceeds bound")
	}
...
func Insert(ctx context.Context, tx *sql.Tx, action domain.ActionID, admission Admission) error {
	...
	_, err = tx.ExecContext(ctx, "INSERT INTO capture_admissions(action_id,payload) VALUES(?,?) ON CONFLICT(action_id) DO UPDATE SET payload=excluded.payload", action, data)
	return err
}
