...
CREATE TABLE submissions(request_id TEXT PRIMARY KEY, kind TEXT NOT NULL CHECK(kind IN ('building','owned_draft','caravan_departure','quest_accept','settlement_gift','quest_fulfill','travel_caravan','trade','zone_create','zone_edit','research_select','resource_policy','build_room')), colony TEXT NOT NULL, load_token TEXT NOT NULL, map_id INTEGER NOT NULL, plan_id TEXT NOT NULL UNIQUE REFERENCES plans(id), action_id TEXT NOT NULL UNIQUE REFERENCES actions(id), revision TEXT NOT NULL) STRICT;
...
func load(ctx context.Context, tx *sql.Tx, id domain.PlanID) (PlanState, error) {
	...
	rows, err := tx.QueryContext(ctx, "SELECT id,kind,definition,x,z,rotation,stuff,pawn,target,draft_action,work_payload,zone_payload,zone_edit_payload,bill_payload,caravan_payload,settlement_gift_payload,quest_fulfill_payload,wall_removal_payload,production_policy_payload,building_temperature_payload,travel_caravan_payload,trade_payload,mood_relief_payload,ordinal FROM actions WHERE plan_id=? ORDER BY ordinal", id)
	if err != nil {
		return PlanState{}, err
	}
