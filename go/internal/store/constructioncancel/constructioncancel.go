// Package constructioncancel holds the construction-cancel action family's
// admission record, validation and load logic, split out of internal/store for
// independent build/test caching -- mirroring internal/store/zoneedit.
package constructioncancel
...
// Admission is the exact native pending-construction target one inspection
// found for this action, plus its CAS snapshot token.
//
// Unlike zoneedit.Admission the identity is not derivable from the action: a
// pending construction order's thing ID changes when its blueprint becomes a
// frame, so the cell/definition/material triple in the action is what stays
// stable and ThingID is the identity that exact inspection resolved it to.
// Def and Stuff are recorded alongside so a reloaded admission can be checked
// against the action it belongs to rather than trusted.
...
func ValidateAdmission(a domain.Action, p domain.Progress, admission Admission) error {
	...
	cancel, ok := a.ConstructionCancel()
	if !ok || cancel.Definition() != admission.Def || cancel.Material() != admission.Stuff ||
		idShaped(admission.ThingID) != nil || idShaped(admission.SnapshotToken) != nil ||
		admission.Snapshot.Native == 0 || admission.Snapshot.Direction == 0 {
		return errors.New("invalid construction cancel admission")
	}
	return nil
}
...
func LoadAdmission(ctx context.Context, tx *sql.Tx, a domain.Action, p domain.Progress) (Admission, bool, error) {
	var data []byte
	if err := tx.QueryRowContext(ctx, "SELECT payload FROM construction_cancel_admissions WHERE action_id=?", a.ID()).Scan(&data); err != nil {
	...
	if len(data) > 32768 {
		return Admission{}, false, errors.New("construction cancel admission exceeds bound")
	}
...
func Insert(ctx context.Context, tx *sql.Tx, action domain.ActionID, admission Admission) error {
	...
	_, err = tx.ExecContext(ctx, "INSERT INTO construction_cancel_admissions(action_id,payload) VALUES(?,?) ON CONFLICT(action_id) DO UPDATE SET payload=excluded.payload", action, data)
	return err
}
