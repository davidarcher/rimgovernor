// Package melee holds the melee-attack action family's admission record,
...
func Insert(ctx context.Context, tx *sql.Tx, action domain.ActionID, v Admission) error {
	...
	if err != nil || len(data) > 32768 {
		return errors.New("melee admission exceeds bound")
	}
	_, err = tx.ExecContext(ctx, "INSERT INTO melee_admissions(action_id,payload) VALUES(?,?) ON CONFLICT(action_id) DO UPDATE SET payload=excluded.payload", action, data)
	return err
}
