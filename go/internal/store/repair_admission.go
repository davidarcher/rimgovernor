...
type ActionRepairAdmission struct {
	Action    domain.ActionID
	Admission RepairAdmission
}

func validateRepairAdmission(a domain.Action, p domain.Progress, admission RepairAdmission) error {
	...
	repair, ok := a.Repair()
	if !ok || repair.Pawn() != admission.Pawn || repair.Structure() != admission.Structure || repair.Cell() != admission.Cell || submissionID(admission.PawnSnapshotToken) != nil || submissionID(admission.StructureSnapshotToken) != nil {
		return errors.New("invalid repair admission")
	}
	return nil
}

// PrepareRepair atomically records the exact pawn/structure CAS evidence and
...
func (s *Store) PrepareRepair(ctx context.Context, plan domain.PlanID, action domain.ActionID, admission RepairAdmission) (domain.Progress, error) {
	tx, err := s.begin(ctx)
	if err != nil {
	...
	if err = validateRepairAdmission(a, p, admission); err != nil {
		return domain.Progress{}, err
	}
	v := p.View()
	...
	for _, old := range state.RepairAdmissions {
	...
	if _, err = tx.ExecContext(ctx, "INSERT INTO repair_admissions(action_id,payload) VALUES(?,?) ON CONFLICT(action_id) DO UPDATE SET payload=excluded.payload", action, data); err != nil {
		return domain.Progress{}, err
	}
...
func loadRepairAdmission(ctx context.Context, tx *sql.Tx, a domain.Action, p domain.Progress) (RepairAdmission, bool, error) {
	var data []byte
	if err := tx.QueryRowContext(ctx, "SELECT payload FROM repair_admissions WHERE action_id=?", a.ID()).Scan(&data); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return RepairAdmission{}, false, nil
		}
		return RepairAdmission{}, false, err
	}
	var admission RepairAdmission
	if len(data) > 32768 {
		return RepairAdmission{}, false, errors.New("repair admission exceeds bound")
	}
	...
	if err := decoder.Decode(new(json.RawMessage)); err != io.EOF {
		return RepairAdmission{}, false, errors.New("trailing admission data")
	}
	...
	if !bytes.Equal(data, canonical) {
		return RepairAdmission{}, false, errors.New("noncanonical admission record")
	}
	if err = validateRepairAdmission(a, p, admission); err != nil {
		return RepairAdmission{}, false, fmt.Errorf("invalid action %q admission: %w", a.ID(), err)
	}
	v := p.View()
	if (v.Stage == domain.Prepared || v.Attempt > 0) && !admission.Snapshot.Matches(v.Snapshot) {
		return RepairAdmission{}, false, errors.New("admission and progress authority disagree")
	}
	if (v.Unresolved || v.Stage == domain.Completed || v.Stage == domain.Unsuccessful) && admission.Tick > v.Tick {
		return RepairAdmission{}, false, errors.New("admission is newer than dispatched progress")
	}
