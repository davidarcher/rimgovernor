...
type RepairAdmission = repair.Admission
type ActionRepairAdmission = repair.ActionAdmission
...
func (s *Store) PrepareRepair(ctx context.Context, plan domain.PlanID, action domain.ActionID, admission RepairAdmission) (domain.Progress, error) {
	...
	if err = repair.ValidateAdmission(a, p, admission); err != nil {
		return domain.Progress{}, err
	}
	v := p.View()
	...
	if err = repair.Insert(ctx, tx, action, admission); err != nil {
		return domain.Progress{}, err
	}
	if prepare {
