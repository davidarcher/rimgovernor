...
func (s *Store) PrepareGearReplace(ctx context.Context, plan domain.PlanID, action domain.ActionID, admission GearReplaceAdmission) (domain.Progress, error) {
	tx, err := s.begin(ctx)
	if err != nil {
	...
	for _, old := range state.GearReplaceAdmissions {
