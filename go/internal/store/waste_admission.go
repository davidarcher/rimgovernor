...
type ActionWasteAdmission struct {
	Action    domain.ActionID
	Admission WasteAdmission
}

func validateWasteAdmission(a domain.Action, p domain.Progress, admission WasteAdmission) error {
	...
	waste, ok := a.Waste()
	if !ok || waste.Pawn() != admission.Pawn || waste.Target() != admission.Target || waste.Cell() != admission.Cell || submissionID(admission.PawnSnapshotToken) != nil || submissionID(admission.TargetSnapshotToken) != nil {
		return errors.New("invalid waste admission")
	}
	return nil
}

// PrepareWaste atomically records the exact pawn/target CAS evidence and
...
func (s *Store) PrepareWaste(ctx context.Context, plan domain.PlanID, action domain.ActionID, admission WasteAdmission) (domain.Progress, error) {
	tx, err := s.begin(ctx)
	if err != nil {
	...
	if err = validateWasteAdmission(a, p, admission); err != nil {
		return domain.Progress{}, err
	}
	v := p.View()
	...
	for _, old := range state.WasteAdmissions {
	...
	if _, err = tx.ExecContext(ctx, "INSERT INTO waste_admissions(action_id,payload) VALUES(?,?) ON CONFLICT(action_id) DO UPDATE SET payload=excluded.payload", action, data); err != nil {
		return domain.Progress{}, err
	}
...
func loadWasteAdmission(ctx context.Context, tx *sql.Tx, a domain.Action, p domain.Progress) (WasteAdmission, bool, error) {
	var data []byte
	if err := tx.QueryRowContext(ctx, "SELECT payload FROM waste_admissions WHERE action_id=?", a.ID()).Scan(&data); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return WasteAdmission{}, false, nil
		}
		return WasteAdmission{}, false, err
	}
	var admission WasteAdmission
	if len(data) > 32768 {
		return WasteAdmission{}, false, errors.New("waste admission exceeds bound")
	}
	...
	if err := decoder.Decode(new(json.RawMessage)); err != io.EOF {
		return WasteAdmission{}, false, errors.New("trailing admission data")
	}
	...
	if !bytes.Equal(data, canonical) {
		return WasteAdmission{}, false, errors.New("noncanonical admission record")
	}
	if err = validateWasteAdmission(a, p, admission); err != nil {
		return WasteAdmission{}, false, fmt.Errorf("invalid action %q admission: %w", a.ID(), err)
	}
	v := p.View()
	if (v.Stage == domain.Prepared || v.Attempt > 0) && !admission.Snapshot.Matches(v.Snapshot) {
		return WasteAdmission{}, false, errors.New("admission and progress authority disagree")
	}
	if (v.Unresolved || v.Stage == domain.Completed || v.Stage == domain.Unsuccessful) && admission.Tick > v.Tick {
		return WasteAdmission{}, false, errors.New("admission is newer than dispatched progress")
	}
