...
type DraftAdmission = draft.Admission
type ActionDraftAdmission = draft.ActionAdmission
...
func (s *Store) PrepareDraft(ctx context.Context, plan domain.PlanID, action domain.ActionID, admission DraftAdmission) (domain.Progress, error) {
	...
	if err = draft.ValidateAdmission(a, p, admission); err != nil {
		return domain.Progress{}, err
	}
	v := p.View()
	...
	if err = draft.Insert(ctx, tx, action, admission); err != nil {
		return domain.Progress{}, err
	}
