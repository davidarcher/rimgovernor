...
type HusbandryAdmission struct {
...
type ActionHusbandryAdmission struct {
	Action    domain.ActionID
	Admission HusbandryAdmission
}

func validateHusbandryAdmission(a domain.Action, p domain.Progress, admission HusbandryAdmission) error {
...
// PrepareHusbandry atomically records the exact animal/census CAS evidence
...
func (s *Store) PrepareHusbandry(ctx context.Context, plan domain.PlanID, action domain.ActionID, admission HusbandryAdmission) (domain.Progress, error) {
	tx, err := s.begin(ctx)
	if err != nil {
	...
	if err = validateHusbandryAdmission(a, p, admission); err != nil {
		return domain.Progress{}, err
	}
	v := p.View()
	...
	for _, old := range state.HusbandryAdmissions {
...
func loadHusbandryAdmission(ctx context.Context, tx *sql.Tx, a domain.Action, p domain.Progress) (HusbandryAdmission, bool, error) {
	var data []byte
	if err := tx.QueryRowContext(ctx, "SELECT payload FROM husbandry_admissions WHERE action_id=?", a.ID()).Scan(&data); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return HusbandryAdmission{}, false, nil
		}
		return HusbandryAdmission{}, false, err
	}
	var admission HusbandryAdmission
	if len(data) > 32768 {
		return HusbandryAdmission{}, false, errors.New("husbandry admission exceeds bound")
	}
	...
	if err := decoder.Decode(new(json.RawMessage)); err != io.EOF {
		return HusbandryAdmission{}, false, errors.New("trailing admission data")
	}
	...
	if !bytes.Equal(data, canonical) {
		return HusbandryAdmission{}, false, errors.New("noncanonical admission record")
	}
	if err = validateHusbandryAdmission(a, p, admission); err != nil {
		return HusbandryAdmission{}, false, fmt.Errorf("invalid action %q admission: %w", a.ID(), err)
	}
	v := p.View()
	if (v.Stage == domain.Prepared || v.Attempt > 0) && !admission.Snapshot.Matches(v.Snapshot) {
		return HusbandryAdmission{}, false, errors.New("admission and progress authority disagree")
	}
	if (v.Unresolved || v.Stage == domain.Completed || v.Stage == domain.Unsuccessful) && admission.Tick > v.Tick {
		return HusbandryAdmission{}, false, errors.New("admission is newer than dispatched progress")
	}
