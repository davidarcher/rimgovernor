// Package equip holds the equip action family's admission record,
...
func ValidateAdmission(a domain.Action, p domain.Progress, admission Admission) error {
	...
	e, ok := a.Equip()
	if !ok || e.Pawn() != admission.Pawn || e.Thing() != admission.Thing || e.Definition() != admission.Definition || e.Cell() != admission.Cell || idShaped(admission.PawnSnapshotToken) != nil || idShaped(admission.ThingSnapshotToken) != nil {
...
func LoadAdmission(ctx context.Context, tx *sql.Tx, a domain.Action, p domain.Progress) (Admission, bool, error) {
	...
	if len(data) > 32768 {
		return Admission{}, false, errors.New("equip admission exceeds bound")
	}
...
func Insert(ctx context.Context, tx *sql.Tx, action domain.ActionID, admission Admission) error {
	...
	_, err = tx.ExecContext(ctx, "INSERT INTO equip_admissions(action_id,payload) VALUES(?,?) ON CONFLICT(action_id) DO UPDATE SET payload=excluded.payload", action, data)
	return err
}
