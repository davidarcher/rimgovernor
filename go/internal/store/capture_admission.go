...
type CaptureAdmission = capture.Admission
type ActionCaptureAdmission = capture.ActionAdmission

// PrepareCapture atomically records the exact capturer/patient CAS evidence
// and prepares pending work, mirroring PrepareRescue exactly.
func (s *Store) PrepareCapture(ctx context.Context, plan domain.PlanID, action domain.ActionID, admission CaptureAdmission) (domain.Progress, error) {
	tx, err := s.begin(ctx)
	if err != nil {
	...
	if err = capture.ValidateAdmission(a, p, admission); err != nil {
		return domain.Progress{}, err
	}
	v := p.View()
	...
	for _, old := range state.CaptureAdmissions {
	...
	if err = capture.Insert(ctx, tx, action, admission); err != nil {
		return domain.Progress{}, err
	}
	if prepare {
