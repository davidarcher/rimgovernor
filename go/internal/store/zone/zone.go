// Package zone holds the zone-create action family's admission record,
...
func LoadAdmission(ctx context.Context, tx *sql.Tx, a domain.Action, p domain.Progress) (Admission, bool, error) {
	...
	if len(data) > 32768 {
		return Admission{}, false, errors.New("zone admission exceeds bound")
	}
...
func Insert(ctx context.Context, tx *sql.Tx, action domain.ActionID, admission Admission) error {
	...
	_, err = tx.ExecContext(ctx, "INSERT INTO zone_admissions(action_id,payload) VALUES(?,?) ON CONFLICT(action_id) DO UPDATE SET payload=excluded.payload", action, data)
	return err
}
