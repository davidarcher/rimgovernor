...
function response(url: string) {if (url.endsWith('/camera')) return reply(values.camera); if (url.endsWith('/selection')) return reply(values.selection); return reply(values.colonists);}
...
it('reads three fixed GETs and renders unknown/partial/known empty facts without controls', async () => {
 ...
 expect(fetcher.mock.calls.map(v => v[0])).toEqual(['/api/presentation/camera', '/api/presentation/selection', '/api/presentation/colonists']);
});
