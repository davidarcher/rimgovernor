...
export default function FoodPlanPanel({active}: {active: boolean}) {
  ...
  return <section className="observation-panel food-plan" aria-label="Food channels">
    ...
    {!p ? <p>{reading ? 'Food plan unavailable for the current tick.' : 'Waiting for the food plan.'}</p> : <>
      ...
      {p.petShortfalls.length ? <ul className="food-pets">{p.petShortfalls.map(pet => <li key={pet.id}><strong title={pet.id}>{pet.label ?? pet.id}</strong> · {rate(pet.runwayDays)} days of food · needs {rate(pet.nutritionPerDay)} nutrition/day</li>)}</ul> : <p>No pet below the minimum food runway.</p>}
