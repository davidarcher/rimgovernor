...
function object(value: unknown, keys: readonly string[]): Record<string, unknown> {
  if (!isObject(value) || Object.keys(value).length !== keys.length || keys.some(key => !Object.hasOwn(value, key))) throw Error('Invalid building response fields');
  return value;
}
...
function choice<T extends string>(value: unknown, values: readonly T[]): T {const result = values.find(item => item === value); if (result === undefined) throw Error('Unknown building response variant'); return result;}
...
export function acquireBuilding(token: string, body: AcquireRequest, signal?: AbortSignal): Promise<ControlReply> {return controlRequest('/api/buildings/control/acquire', signal, token, body);}
export function manualBuilding(token: string, body: ManualRequest, signal?: AbortSignal): Promise<ControlReply> {return controlRequest('/api/buildings/control/manual', signal, token, body);}
