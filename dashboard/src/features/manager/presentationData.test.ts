...
it('rejects malformed identities, numeric bounds, variants and collection evidence', () => {
 for (const camera of [{context: {...context, tick: 42}}, {context: {...context, nativeGeneration: '18446744073709551616'}}, {context, rootSize: Infinity}, {context, zoomRootSize: 0}, {context, viewRect: {minX: 4, maxX: 2}}, {context, extra: true}, {context: {...context, identity: {...context.identity, loadToken: '\ud800'}}}]) expect(() => readCamera({camera})).toThrow();
 expect(() => readCamera({failure: {code: 'UNKNOWN'}})).toThrow();
 expect(() => readSelection({selection: {context, selectedObjects: [{id: 'x'}, {id: 'x'}]}})).toThrow();
 expect(() => readSelection({selection: {context, listing: {complete: true}}})).toThrow();
