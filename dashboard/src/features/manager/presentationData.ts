...
function fingerprint(v: unknown): string {const s = text(v); if ([...s].length > 4096) throw Error('Oversized presentation fingerprint'); return s;}
...
function cell(v: unknown, reader = integer): Cell {const c = object(v, ['x', 'z']); return {x: optional(c.x, reader), z: optional(c.z, reader)};}
