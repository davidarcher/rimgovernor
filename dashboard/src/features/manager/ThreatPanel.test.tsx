...
it('refuses a census whose figures are not finite non-negative numbers', () => {
  ...
  expect(readThreatStatus({...census, shrines: null}).shrines).toBeNull();
  expect(readThreatStatus({...census, buildTier: null}).buildTier).toBeNull();
  expect(readThreatStatus({...census, buildTier: undefined}).buildTier).toBeNull();
  expect(() => readThreatStatus({...census, buildTier: 3})).toThrow('buildTier');
  expect(() => readThreatStatus({...census, shrines: [{...census.shrines[0], caskets: -1}]})).toThrow('caskets');
});
