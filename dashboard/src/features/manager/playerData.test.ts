...
const record = {requestId:'acquire',kind:'acquire',expected:world,planId:'plan',revision:'9007199254740993',phase:'granted',nativeGeneration:'2'};
...
it('keeps uint64 strings and historical grants separate from actual permission',()=>{
 const value=readControl(reply);expect(value.record?.revision).toBe('9007199254740993');expect(value.state.enabled).toBe(false);
...
it('rejects unknown fields, missing facts and invalid typed variants',()=>{
 for(const value of [{...reply,extra:1},{...reply,state:{enabled:true,observationKnown:false,generation:null}},{...reply,record:{...record,revision:'18446744073709551616'}},{...reply,record:{...record,revision:1}},{...reply,record:{...record,phase:'invented'}},{...reply,record:{...record,phase:'uncertain'}},{...reply,record:{...record,kind:'manual'}}]) expect(()=>readControl(value)).toThrow();
...
it('retains a typed uncertain record on HTTP503 and sends the exact token/CAS',async()=>{
 ...
 const request={requestId:'acquire',expected:world,planId:'plan',revision:record.revision};
