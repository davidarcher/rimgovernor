...
import {resumeControl, readBuilding, readControl, readPlayerSession, readSubmission, type ControlReply} from './playerData';
...
const record = {requestId:'resume',kind:'resume',expected:world,phase:'running',nativeGeneration:'2'};
...
it('keeps uint64 strings and historical grants separate from actual permission',()=>{
 const value=readControl(reply);expect(value.record?.phase).toBe('running');expect(value.state.enabled).toBe(false);
 expect(readSubmission({requestId:'submit',expected:world,building,planId:'plan',actionId:'action',revision}).revision).toBe(revision);
});
it('rejects unknown fields, missing facts and invalid typed variants',()=>{
 for(const value of [{...reply,extra:1},{...reply,state:{enabled:true,observationKnown:false,generation:null}},{...reply,record:{...record,planId:'plan'}},{...reply,record:{...record,nativeGeneration:'0'}},{...reply,record:{...record,phase:'invented'}},{...reply,record:{...record,phase:'uncertain'}},{...reply,record:{...record,kind:'pause'}},{...reply,record:{...record,kind:'pause',phase:'paused'}},{...reply,record:{...record,phase:'paused',nativeGeneration:'0'}}]) expect(()=>readControl(value)).toThrow();
...
it('retains a typed uncertain record on HTTP503 and sends the exact token/CAS',async()=>{
 ...
 const request={requestId:'resume',expected:world};
 expect(await resumeControl('secret',request)).toEqual(uncertain);
 const options=fetcher.mock.calls[0];expect(options).toBeDefined();
 expect(fetcher).toHaveBeenCalledWith('/api/player/control/resume',expect.objectContaining({method:'POST',headers:{'Content-Type':'application/json','X-RimGovernor-Player':'secret'},body:JSON.stringify(request)}));
});
