...
const outcome=(e:any)=>['project_dependency','project_deadline','risk_transition','project_suspended','project_resumed','arbitration','spatial_plan','project_cancelled','work_outcome','error','escalation'].includes(e.kind)||(e.kind==='execution'&&(Object.values(e.outcomes||{}).some(n=>Number(n)>0)||e.blockers?.length));
...
function EventRow({event:e,technical=false}:{event:any;technical?:boolean}){
 const labels:Record<string,string>={project_dependency:'Prerequisites',project_deadline:'Target missed',risk_transition:'Colony change',project_suspended:'Project paused',project_resumed:'Project resumed',execution:'Orders',work_outcome:'Verified',arbitration:'Decision',spatial_plan:'Layout planned',project_cancelled:'Project cancelled',error:'Blocked',escalation:'Needs a decision'};
