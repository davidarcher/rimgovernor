...
type Frame = { source: string; frame: number; width: number; height: number; captured: number; session: string; encoding?: string };
...
export default function GameVideo({ session, viewer, enabled, snapshot, children, owner, onError }: Props) {
  ...
  useEffect(() => { displayed.current = null; setHasFrame(false); failures.current = 0; }, [session]);
  ...
  useEffect(() => {
    ...
    socket.onmessage = async event => {
      ...
      try {
        ...
        // This acknowledgement follows the next browser paint, not just packet arrival.
        requestAnimationFrame(() => requestAnimationFrame(() => {
          if (stopped) return;
          socket.send(JSON.stringify({ frame: frame.frame, displayed: Date.now() / 1000 }));
